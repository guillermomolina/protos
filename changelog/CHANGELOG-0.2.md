# Protos Implementation Changelog — 0.2.x

Archived from [`../CHANGELOG.md`](../CHANGELOG.md).

## 0.2.494-SNAPSHOT

- Ratify `D129 — Test Tool per-case project-tree corpus provisioning contract` (GitHub #501) as Candidate A′. Keep per-case project-tree provisioning inside CorpusBinding/TestPlan materialization with explicit isolated CaseAuthority, deterministic teardown, no ambient discovery, and bounded-parallel-safe isolation. Do not introduce a new ExecutionRequirementId or D077/D098 resource dimension merely for project-tree fixtures. Governance/tooling decision only.\n\n- Ratify `D128 — VSIX reproducibility identity and canonical artifact contract` (GitHub #500) as Candidate C″ after explicit project-owner approval and an expanded audit across Reproducible Builds/SOURCE_DATE_EPOCH, Maven, Gradle, Go, Nix/NixOS, Bazel, SLSA, Docker/BuildKit, Cargo, npm, Debian/Python wheels, JDK/JAR, Nix VS Code extension consumption and VSCE/yazl/yauzl behavior. Define `VSIX_REPRODUCIBLE=YES` as both exact packaged-member reproducibility and byte-for-byte canonical VSIX reproducibility. Keep VSCE as the sole semantic/package-content authority; canonicalize transport only using source-commit-derived `SOURCE_DATE_EPOCH`, lexicographic entry order, fixed modes/attributes and ZIP STORE so compressor implementation is not a hidden artifact input. Require two independent clean-worktree proofs and unchanged semantic member paths/bytes. Governance/editor-tooling only: no Protos specification, runtime/native boundary, Maven implementation version or VS Code extension version change; LM009-I1-C becomes READY.

- Implement `LM009-I1-B — bounded VSIX package contents` (GitHub #474) under ratified D127 Candidate B′. Add package-root `license.txt` as an exact copy of the repository APL-1.0 `LICENSE.TXT`; generate and retain deterministic `THIRD_PARTY_NOTICES.txt` from the actual npm package inputs in the esbuild production metafile; add an explicit `.vscodeignore`; package with pinned `vsce --no-dependencies`; and validate the actual VSIX against an exact eight-file extension payload, rejecting production `node_modules`, tests, fixtures, raw editor sources, build scripts, lockfile and esbuild metadata. I1-B closes with one bounded packaging smoke; I1-C is READY for clean-checkout reproducibility/content closure. Editor tooling only: no Protos specification, runtime/native boundary, Maven implementation version or VS Code extension version change.

- Add a retrospective `docs/news/` entry for Protos's first merged external community contribution, PR #315 by `jabrailkhalil` on 2026-09-11, and document the intended split between GitHub Discussions Announcements (conversation/community visibility) and `docs/news/` (durable versioned milestone archive). PERF006 remains the README `Latest milestone`; no Issue family, Project item, specification, executable or implementation-version change.

- Implement `LM009-I1-A — deterministic VS Code client bundle foundation` (GitHub #474) under ratified D127 Candidate B′. Commit the npm lockfile, pin esbuild `0.28.2` and `@vscode/vsce` `3.9.2`, switch the production extension entry to generated `dist/extension.js`, bundle the local debug adapter plus `vscode-languageclient` JavaScript closure while preserving host-provided `vscode` as external, ignore generated `dist/`, and add a fail-closed bundle/lockfile validator. I1 remains IN_PROGRESS; package-root APL/third-party notices, bounded VSIX contents and reproducibility closure remain I1-B/I1-C. Editor tooling only: no Protos specification, runtime, native boundary, Maven implementation version or VS Code extension version change.

- Publish the first public Protos milestone/news entry for the completed `PERF006` runtime-integrity track and surface it as the README `Latest milestone`: summarize the C′ production cutover, exact-source optimizer-vs-fallback steady-state evidence (1.06x–1.73x across all five retained workloads), disappearance of the historical replay/HashMap/deoptimization pathology, explicit historical-speedup non-claim, and independent PERF008 follow-up. Establish `docs/news/` as a public-facing milestone archive distinct from actionable GitHub Issues. Governance/documentation only; no specification, executable or implementation-version change.

- Ratify `D127 — VS Code extension dependency bundling and runtime distribution boundary` (GitHub #497) as Candidate B′ after explicit project-owner approval and an exhaustive comparison across VS Code platform guidance, rust-analyzer, VS Code Go, clangd VS Code, Ruff, Red Hat Java/XML, Haskell/OCaml and alternative raw-node_modules / activation-download / hand-written-client patterns. Require a committed npm lockfile, clean `npm ci`, pinned build tooling, a bundled JavaScript client closure with host-provided `vscode` external, no production `node_modules` tree in the VSIX, no activation-time dependency fetch and a hard external `protos.runtime.executable` boundary for Run/Debug/language-server. Require package-root APL-1.0 material, third-party notices, an explicit package-content boundary and fail-closed content regression. Governance/editor-tooling only: no Protos specification, runtime implementation, native-boundary, extension-version or implementation-version change; LM009-I1 becomes READY.

- Ratify `D126 — Test Tool corpus identity and plan-source binding contract` (GitHub #495) as Candidate D′ after explicit project-owner approval and an exhaustive comparison across Bazel, Buck2, Pants, Gradle JVM Test Suites, Cargo/libtest, SwiftPM, Go, Microsoft.Testing.Platform, JUnit Platform, pytest, CTest, Meson, Maven Surefire, Jest, Vitest and Erlang Common Test. Require exactly one mandatory stable logical `CorpusId` per executable suite leaf in a namespace separate from `SuiteId` and `ExecutionRequirementId`; resolve it by exact lookup through an invocation-scoped immutable host `CorpusBinding` registry, validate all corpus bindings and materialize current plans before scheduling, and keep source authority/TestPlan materialization out of `ExecutionBinding`. Do not persist `PlanLoaderId` initially; forbid path/URL/module-path/content-digest/host implementation identity in corpus descriptors; reserve `protos/corpus/conformance`, `protos/corpus/actor`, `protos/corpus/group` and `protos/corpus/package-toml` as generation-1 repository-owned CorpusIds. Governance/tooling only: no executable, specification, native-boundary or implementation-version change.

- Close `PERF006-D` and the parent `PERF006` runtime-integrity track after retained exact-source optimizer/fallback evidence and current JDK-25 structural diagnostics. Controlled D2 evidence shows the selected HotSpotTruffleRuntime faster in steady state for all five retained canonical workloads (about 1.06x–1.73x median) while retaining a real startup cost; D3 proves the historical replay pathology is absent (`HashMap$KeyIterator.next` 0%, old replay-cleanup symbols absent, 191 JVM / 6 Truffle deoptimizations versus the historical ~1.825M storm) and confirms real guest compilation. Do not attribute historical ~471.985s→77.613s or ~20min→~8min observations solely to the optimizer because C-prime/replay architecture and harness also changed. The new `CachedBytecodeNode.continueAt` profile lead is transferred to independent PERF008 / GitHub #496. This closure also discharges PERF006-B's deferred retained/full-validation debt using the already-published C3 267/267 retained and 1913/1913 full Maven evidence. Governance/documentation only; no executable, specification, runtime, Graal/Truffle or implementation-version change.

- Implement `LM009-H1 — D124 static references generation 1` (GitHub #489). Advertise standard LSP references from the dedicated Protos language server, accept only an exact singleton D110-backed seed, and report only occurrences whose D110 complete finite proof result contains that target. Reuse D110 as the semantic authority for candidate membership; same-name scanning is only a cost filter, with no workspace-symbol/text/path/runtime fallback. Support Closure parameters and generation-1 match Binder/Alias origins, exact `includeDeclaration`, canonical ProjectBinding source authority, current-overlay freshness, deterministic ordering/deduplication, and fail-closed behavior after opaque-effect barriers. Defer hover, completion and signature help to paused LM010 / GitHub #493. Validation is `EXECUTABLE_IMPACT` / `FOCAL_BOUNDED`; broad H closure validation remains H5 while GITHUB017 temporarily suspends automatic full test CI during TOOL005. Implementation version becomes `0.2.494-SNAPSHOT`.

## 0.2.493-SNAPSHOT

- Publish `LIB010-E1 — comment/control + quote-run hardening` under `AUD005` / GitHub #451. Enforce the already-ratified TOML 1.1 comment character contract by rejecting forbidden C0 controls and DEL while retaining horizontal TAB; replace the remaining self-recursive `appendQuotes` helper with bounded iterative traversal so the scanner's no-input-proportional-host-recursion property is structural rather than dependent on quote-run call-site limits. Add retained focal evidence covering every forbidden C0 comment byte, DEL, permitted TAB, an adversarial 16,384-quote run, and a source guard against reintroducing recursive quote appending. Mark `LIB010-E1` CLOSED and `LIB010-E2` READY while final LIB010 closure remains deferred to AUD005. No public TOML API, semantic node shape, D104/D109 contract, D087 private TOML 1.0 behavior, specification, Java/native boundary or composite-constructor policy changes. Implementation version becomes `0.2.493-SNAPSHOT`; full-suite/conformance closure remains owned by LIB010-E2.

## 0.2.467-SNAPSHOT

- Ratify `D124 — Static references identity, ambiguity and completeness contract` (GitHub #491) as Candidate B′ after explicit project-owner approval and an exhaustive comparison across LSP, rust-analyzer, Pyright, TypeScript/tsserver, clangd, gopls, SourceKit-LSP, Eclipse JDT LS, clojure-lsp, LuaLS, ElixirLS, Jedi, Ruby LSP, Self and Pharo/Smalltalk. Require an exact singleton seed; return only occurrences whose complete finite proof-set contains that selected target; treat the result as a sound monotonic under-approximation; reject ambiguous-seed union, same-name/workspace/fuzzy/path/runtime fallback and any rename-completeness implication. Generation 1 remains singleton/scope-first and may expand monotonically. Governance/tooling only: no executable, specification, native-boundary or implementation-version change.

- Ratify `D125 — Test Tool suite execution-requirement identity and host binding contract` (GitHub #490) as Candidate B′ after explicit project-owner approval and an exhaustive comparison across Kubernetes DRA, Bazel, Buck2, CTest, Meson, Gradle JVM Test Suites, Maven Surefire/Failsafe, pytest/xdist, JUnit Platform, Microsoft.Testing.Platform, Erlang Common Test, ExUnit, Cargo/libtest and SwiftPM. Require exactly one mandatory atomic logical `ExecutionRequirementId` on every executable suite leaf, with no default or inheritance; resolve it by exact lookup through one invocation-scoped immutable host binding registry and fail closed before scheduling on missing/unknown/duplicate bindings. Keep SuiteId, corpus identity, D077/D098 resources, presentation and physical placement orthogonal. Reserve `protos/test/ordinary`, `protos/test/actor`, `protos/test/group` and `protos/test/package` as generation-1 repository-owned execution-requirement IDs. Governance/tooling only: no executable, specification, native-boundary or implementation-version change.

- Ratify `D123 — Test Tool SuiteId lexical identity and namespace contract` (GitHub #488) as Candidate G′ after explicit project-owner approval and an exhaustive comparison across Microsoft.Testing.Platform, JUnit Platform, Bazel, Buck2, Meson, Gradle Test Suites, Go testing, Rust Cargo/libtest, Erlang Common Test, Pharo/SUnit, pytest/unittest, Maven Surefire/Failsafe, Jest, Vitest, Deno/Node, ExUnit, NUnit, GoogleTest, PHPUnit and tox. Select explicit assigned logical namespace IDs with canonical `segment[/segment...]` grammar; `/` is namespace segmentation only, never filesystem path or graph ancestry. Reserve `protos/repository`, `protos/conformance`, `protos/actor`, `protos/group` and `protos/package-toml` as the generation-1 repository-owned IDs, keep display names separate, and forbid deriving identity from paths, repository/domain locators, host implementations, execution requirements or D098 provider/profile. Governance/tooling only: no executable, specification, native-boundary or implementation-version change.

- Publish `DOC005-D — Test Tool results, diagnostics, exit status and CI recipes`: document D116 N-plan completed aggregation, the current `0` all-healthy / `1` completed guest-test failure / `3` infrastructure-aborted classification, exit-`3` precedence with preserved guest evidence, bounded infrastructure-abort stderr diagnostics, Tool/configuration Error versus completed-test-failure distinction, progress/output channels and fail-closed CI recipes. The DOC005-C audit also records the current missing public `ResourceRequirements.loadFromCorpus(...)` wiring as `BLOCKED_BY_TOOL006` (#473) instead of documenting a non-runnable resource recipe. Validation is `GOVERNANCE_DOCUMENTATION_ONLY`; command examples are source-checked but `protos test`, Maven and all focal/full executable suites are not run; no specification, implementation or version change.

- Publish `DOC005-B — Test Tool fundamentals`: document current `protos test` four-plan corpus routing, retained manifest/expectation model, fresh semantic Process isolation, private captured streams, deterministic logical result ordering and positive `--jobs N` outer parallelism, while keeping unimplemented D122/TOOL005 suite routing out of the current user contract. Validation is `GOVERNANCE_DOCUMENTATION_ONLY`; examples are checked against current source/fixtures without running `protos test`, Maven or any focal/full executable test suite; no specification, implementation or version change.

- Publish `DOC005-A — Bundled Tools concept and navigation`: add the maintained programmer-facing Bundled Tools overview, distinguish language/Core, Standard Library, bundled Tool policy and host/runtime integration, document the current Package/Test Tool inventory plus private `self:`/`tool-shared:` confinement without promoting implementation details into public API, and create the DOC005 staged work record. Validation is `GOVERNANCE_DOCUMENTATION_ONLY`; no Maven/focal/full suite, specification, executable, native-boundary or implementation-version change.

- Ratify `D122 — Test Tool composable suite graph and execution-requirement contract` (GitHub #469) as Candidate F after explicit project-owner approval and an expanded comparison across pytest/xdist, Go, Cargo/libtest, JUnit Platform, Gradle, Maven Surefire, Bazel, Buck2, CTest, Meson, Node, Deno, Jest, Vitest, ExUnit, Erlang Common Test, SwiftPM, Microsoft.Testing.Platform, Pharo/SUnit and tox. Select explicit recursive suite composition with stable logical SuiteId, inert serializable descriptors, no authoritative recursive auto-discovery, fail-closed duplicate/cycle/unknown requirement handling, canonical plan-before-scheduling order, and physical-placement independence. Keep suite-side mechanical execution requirements separate from D077/D098 case resources and do not reuse D098 provider-scoped `profile`. Governance/tooling only: no executable, specification, native-boundary or implementation-version change.

- Close `LIB010-D — TOML conformance, stress and baseline closure`, thereby closing the bounded initial `LIB010` public TOML 1.1 semantic surface. Add one retained closure test for official-example/toml-test-oriented parse→encode→parse semantics, fresh-module state independence and the D087/JVM/I/O architecture boundary; compose it with all retained TOML model/parser/parser-stress/encoder evidence; and discharge the inherited `FOCAL_BOUNDED` full-suite debt by reusing exact isolated evidence at `e9c560db697565d82d6f51e5aad329b821daf378` (`make test` -> `mvn clean test`: 1,893 tests, 0 failures, 0 errors, 1 skipped, BUILD SUCCESS, worktree integrity PASS). V9 accepts that evidence only when a strict ancestor/commit-count/changed-path/POM compatibility gate proves the reviewed post-evidence interval is unchanged, then runs the complete current TOML integrated focal set on the publication candidate. No executable library implementation, public API, specification, D087 private TOML 1.0 source, Java/native boundary or implementation version changes. Implementation remains `0.2.467-SNAPSHOT`; future Document, streaming, richer diagnostics, I/O adapters, object binding and TOML dialect migrations remain separate work.

- Close `LIB011-F — adversarial/conformance closure` and the initial LIB011 scope without changing the public API or implementation version. Add authoritative Protos conformance for a 128-option / 256-token command-line specification, deterministic complete help at scale, repeated parsing with no retained occurrence state, and concurrent independent parse/help invocations with no global registry leakage. Reconcile the retained A–E adversarial corpus and Test Tool adoption under one complete Maven-suite closure gate.

- Close `TOOL002-J — final Test Tool validation and architecture reconciliation` and complete the TOOL002 implementation track. Materialize the selected Java-first / Protos-tool-second CI order by retaining impact-aware repository Java/runtime validation as stage one and running the existing real public `protos test --jobs 2` CLI checkpoint explicitly as fail-closed stage two. Add a Tool-owned closure guard that preserves bundled-Protos ownership of D108 execution, D114 infrastructure-abort projection and D116 N-plan final 0/1/3 classification while Java remains a mechanical bootstrap/carrier consumer. Reconcile the scheduler/distribution-capable architecture without selecting deferred report-schema, retry, timeout, sharding/remote-worker, watch, cache, auto-jobs or new provider/resource policy. Closure evidence reuses the immutable full-suite PASS at `e9c560db` (1893 tests, 0 failures, 0 errors, integrity PASS), revalidates the exact intervening executable chain `1a3dfe80` -> `04c8ba9e` -> `875b62c1` with the integrated union of the LIB011-D, PLAT031 buffered-I/O and LIB011-E focal sets, rejects later executable drift while allowing only docs/CHANGELOG and root-project-version-only POM drift, and then runs the J closure guard plus the public Test Tool checkpoint. Test/CI/governance closure only: no specification, runtime implementation, native-boundary or implementation-version change.

- Close `I038 — Matching expression implementation` (GitHub #391) through `I038-D7D — integrated validation and top-level closure`. The complete already-ratified D071–D096 matching surface is now published across parser/canonical IR, AST execution and the production Bytecode backend: ordinary pattern-owned `match(subject)` authority and D096 tri-state outcome handling; binder/wildcard/alias and D088 positional arm binding; D083 capture composition including D103 terminal dynamic capture tails; D084 standard Array exact/remainder matching with pre-child shallow snapshots; D086/D095 standard Map stable association snapshots, ordered query-key resolution, exactness and frozen residue; ordered OR commitment; guards, terminal fresh Error behavior and coverage/redundancy validation. D7A/D7B/D7C establish C-prime suspension/resume parity for matcher, guard, arm body, Array children and Map query/hash/equality/value children without replay. I038-D7D adds no new language semantics or production implementation; it retires the accumulated `FOCAL_BOUNDED` validation debt using complete integrated-suite evidence plus current-head matching and bounded post-evidence impact revalidation after proving that intervening LIB011 CLI, PLAT031 buffered-I/O, and LIB011-E/TOOL002 option-adoption publications do not modify the matching/parser/canonicalization/Bytecode authority surface and after focally revalidating their exact changed seams. Implementation version becomes `0.2.467-SNAPSHOT`.

## 0.2.466-SNAPSHOT

- Close `LIB011-E — Test Tool adoption`. Migrate bundled Test Tool option structure recognition for the already-ratified `--jobs N` and `--resource-catalog PATH` forms onto `std:cli/CommandLine` while preserving TOOL002 policy exactly: unknown arguments (including `--` and unattached foreign options) remain ignored by a Tool-local projection adapter, attached forms remain unsupported/ignored, selected values remain literal, D069 positive-Integer validation/default `jobs=1` stays Tool-owned, and D099/D101 resource-catalog absence/exact PATH selection stays Tool-owned. Add Protos-language conformance evidence plus a thin bundled-tool Java harness. `Main.protos` and host CLI parsing remain unchanged. `LIB011-F` becomes READY. Implementation version becomes `0.2.466-SNAPSHOT`.

## 0.2.464-SNAPSHOT

- Close `LIB011-D — pure help rendering` under ratified D118 Candidate F-prime and D119 Candidate A-prime. Add `CommandLine.renderHelp(rootSpec, commandPath)` as the deterministic path-aware unwrapped plain-text interpretation of canonical CommandSpec, preserving declaration order, required/optional usage policy, positional cardinality notation, next-line descriptions, immediate-child scope, LF separators and no final newline without terminal/output authority. Tighten canonical value metadata so flags require `valueName=null` while value-taking options and all positionals require non-empty `valueName`; no logical-key/type/reflection/generic fallback is introduced. Add authoritative Protos semantic fixtures for exact help output, nested path/failure behavior and the D119 renderability invariant. `LIB011-E` becomes READY. Implementation version becomes `0.2.464-SNAPSHOT`.

## 0.2.463-SNAPSHOT

- Close `TOOL002-I — dependency-ordered resource-aware Test Tool continuation` after the published `I8D5C` atomic public Main/CLI cutover at `cfc877e3ee77bef6938fa045028e0fd88b8128de` (`0.2.463-SNAPSHOT`). D076/D077/D087/D091/D094/D097/D098/D099/D101/D105/D107/D108/D113/D114/D116 remain ratified; I1-I8D5C are published/closed. The public Test Tool now owns resource-aware execution for all currently owned plans, D108 terminal round-drain and SAFE-only release, D113 explicit host registry composition, D114 tagged infrastructure-abort reporting, and D116 N-plan completed 0/1 aggregation with exit 3 infrastructure precedence. Mark `TOOL002-J` READY for final Test Tool closure/reconciliation and broad Java-first / Protos-tool-second validation. Governance/documentation only: no executable, specification, native-boundary or implementation-version change.

- Implement `TOOL002-I8D5C — public Main/CLI resource-aware cutover`. Route all four currently owned Test Tool plans through the D108 resource-aware Runner path with plan-specific ordinary and resourceful execution/inspection bootstraps bound to the correct Prelude. Extend the D113 host resource scope with actor/group/package route pairs while retaining one immutable invocation registry. Implement D116 N-plan completed aggregation in bundled Protos, preserve the historical primary runTuple as the healthy D114 payload, preserve earlier-plan guest evidence and later-plan cutover CaseSpecs on infrastructure abort, add D114 exit classification to TestRunOutcome, and return that carrier from Main. Java consumes only the Protos-selected 0/1/3 classification and emits bounded infrastructure-abort counts; it does not recompute plan policy. Update historical pre-cutover architecture guards and add focal aggregation/classifier/public-routing proofs. Implementation version becomes `0.2.463-SNAPSHOT`.

## 0.2.462-SNAPSHOT
- Ratify `D116 — Test Tool multi-plan completion aggregation and CLI exit contract` (GitHub #443) as Candidate B′ after explicit project-owner approval and exhaustive comparison across Go, Cargo, Maven, Gradle, Bazel, CTest, Meson, pytest, .NET test, JUnit Platform and CI orchestrators. Keep each plan-local runTuple unchanged and retain the historical primary runTuple as the healthy D114 compatibility payload, but make completed invocation classification aggregate every actually executed Test Tool-owned plan. Completed all-healthy maps to exit 0; ordinary guest/test failure in any executed owned plan maps to exit 1; D114 infrastructure-aborted keeps precedence at exit 3 while preserving already-produced guest/test evidence. The primary plan is primary for payload compatibility, not exclusive exit authority. Bundled Protos owns the N-plan aggregation policy; Java consumes the already-decided classification. Future richer multi-plan or event-stream reporting remains additive. Governance/tooling only: no executable, specification, native-boundary or Maven implementation-version change.

- Close `LIB011-C — subcommand traversal and composition` under ratified D115 Candidate C-prime. Extend `CommandLine.parse(spec, arguments)` from the B-stage single-command parser to recursive exact-name command traversal with earliest feasible child boundaries, parent-minimum reservation, D111 prefix allocation, structural current-scope `--` escape, irreversible child ownership, exact child token provenance, and explicit per-scope option ownership. Remove the temporary B-stage subcommand rejection and whole-tree parse-time re-canonicalization so parsing consumes the canonical frozen CommandSpec and indexes only visited scopes, preserving the D115 `O(T + Svisited)` target. Add Protos-language conformance fixtures for D115 boundaries, delimiter escape, option/value ownership and deep recursive frozen results. `LIB011-D` becomes READY. Implementation version becomes `0.2.462-SNAPSHOT`.

## 0.2.461-SNAPSHOT

- Repair two isolated full-suite regressions without changing language semantics. Canonical `Map` structured fast-path recognition now fails closed when a deliberately minimal Prelude omits the public `Map` binding, so provenance/optimization detection returns non-canonical and generic Bytecode execution continues instead of throwing. Reconcile the historical H2B3 closure architecture check with the current Test Tool source shape: `Main.protos` snapshots `arguments: process.args()` before `Options.jobs(arguments)`, and `Options.protos` now carries the D069 default as `jobsValue: 1` inside the shared D069/D099 parser rather than the retired `result: 1` local. PLAT029 non-Task TextWriter routing remains unchanged and is not decided by this slice. No public semantic, specification or API change. Implementation version becomes `0.2.461-SNAPSHOT`.

## 0.2.460-SNAPSHOT

- Ratify `PLAT031 — Buffered I/O operation identity and C-prime ownership integration` (GitHub #442) as Candidate A′ after explicit project-owner approval and exhaustive Truffle/runtime comparison. Standard buffered wrappers converge on one `ProtosIoLifecycle` plus one real `ProtosIoOperation` per accepted read/write/ordinary-flush request; private `Req` records remain adapter-local queue/order/payload metadata and must not duplicate Future identity, semantic commitment, cancellation, close-cutover, terminality or deferred C-prime ownership. `BufferedReader.read` and `BufferedWriter.flush` may use PLAT029 operation-owned C-prime; `BufferedWriter.write` still uses a real operation for lifecycle uniformity. Buffered release/close execution remains governed by PLAT030 and blocked by D112. No specification, executable implementation, public API or implementation-version change.
...
## 0.2.458-SNAPSHOT
- Migrate standard `TextReader.readText` and `readLine` repeated lower-protocol callbacks to the ratified PLAT029 operation-owned C-prime path under `PERF006-B`. Preserve the existing ordered decoder/input queue, progress-oriented text reads, line framing and byte bounds, exact downstream failed-Future Error identity, permanent reader poisoning, pre-commit cancellation, close-cutover discard rules, and the late-read-ahead rule for a cancelled outer read whose already-issued lower `read` later contributes bytes/EOF/failure. One Context-local synthetic C-prime root now owns the complete `source.read -> await Future -> apply leaf outcome -> repeat if more input is required` control extent, so arbitrary guest `source.read` behavior may suspend without replay and backend Future completion never invokes the next guest read inline. A second lower-Future observer is leaf-only evidence custody for terminal aftermath after the outer operation has already lost to cancellation/close; it never schedules guest work or creates Task identity. `TextReader.close`/owning-source release remains outside this slice and blocked by D112/#438. No public semantic or specification change. Implementation version becomes `0.2.458-SNAPSHOT`.

## 0.2.457-SNAPSHOT

- Ratify `D113 — Test Tool production provider-registry integration contract` (GitHub #439) as Candidate B′ after explicit project-owner approval and exhaustive comparison across Kubernetes DRA/device plugins, Nomad, Terraform, Bazel, Buck2, pytest/JUnit, CI runners and WASI/Component capability composition. Select one explicit immutable invocation-scoped host-bootstrap registry mapping exact logical provider IDs to configured transport-neutral adapters; the empty registry is valid/default. Keep provider/profile inert, implementation/version/endpoints/configuration/secrets host-owned, resolution exact/fail-closed and the resource-free path zero-provider/pay-for-use. Reject classpath/ServiceLoader/plugin-directory/PATH/module/catalog-driven ambient discovery, aliases, resource-key inference and fallback chains. Governance/tooling only: no executable, specification, native-boundary or Maven implementation-version change.

- Ratify `D114 — Test Tool infrastructure-abort result and reporting contract` (GitHub #440) as Candidate B′ after explicit project-owner approval and exhaustive comparison across Bazel Remote Execution, pytest, JUnit, Buck2, Go test, Cargo/libtest, CTest, Jenkins, GitHub Actions and Kubernetes orchestration outcomes. Retain the historical healthy `runTuple` inside an outer tagged TestRunOutcome and add a distinct `infrastructure-aborted` outcome carrying ordered executed CaseRuns, ordinary unsupported count, per-attempt infrastructure evidence, exact cutover-not-admitted CaseSpecs and retained unsafe-reservation count. Never fabricate guest failure or skip for infrastructure cutover; preserve guest evidence that already exists. Select CLI classification 0 healthy success, 1 completed guest/test failure, existing 2 usage, 3 infrastructure-aborted and existing 70 unexpected/internal failure; sanitize public infrastructure diagnostics and keep event-stream-first evolution additive. Governance/tooling only: no executable, specification, native-boundary or Maven implementation-version change.

- Implement `LIB011-B — deterministic single-command token parser` under ratified D111 Candidate B-prime. Publish `CommandLine.parse(spec, arguments)` in ordinary Protos with explicit argument snapshotting; exact long/short lookup; `--name`, `--name value`, `--name=value`, `-n value`, zero-arity short clusters and structural `--`; literal consumption of value tokens even when they begin with `-`; occurrence/cardinality enforcement; lossless frozen option/positional provenance; and D111 left-biased maximal-feasible positional allocation with suffix-minimum reservation. Unknown option-like tokens, missing/surplus occurrences and unexpected positionals fail with ordinary Error. Subcommand-bearing specs remain deliberately outside B and are rejected until LIB011-C owns parent/subcommand traversal. Add Protos-first parser fixtures plus updated Java harness/legacy surface assertions. Validation is focal-bounded to the LIB011 harnesses; integrated top-level validation remains deferred. Implementation version becomes `0.2.457-SNAPSHOT`.

## 0.2.456-SNAPSHOT

- Repair the `0.2.453-SNAPSHOT` TextWriter C-prime routing regression: `writeText`, `writeLine`, and `flush` now select the PLAT029 operation-owned C-prime path only through their explicit suspension-capable native entry, rather than inferring C-prime ownership merely from an entered Truffle language Context. This preserves the ratified Bytecode C-prime path while keeping ordinary/direct Java invocation on the historical native state machine, preventing CLI `print(...)` from replaying after Future suspension. Add a focused entered-Context regression proof that ordinary `writeLine` resolves synchronously, performs exactly one target write, and schedules no C-prime operation. No public semantic or specification change. Implementation version becomes `0.2.456-SNAPSHOT`.

## 0.2.455-SNAPSHOT

- Implement `TOOL002-I8D4C3 — Runner SAFE release + D108 round-drain fail-stop`. Add a new private `runD108WithResourcesWithLoader` path while retaining I8C's historical resource-round entrypoint unchanged. Resource-free cases stay on the H async fast path; non-empty ResourceBinding sets project mechanically to the C1/C2 six-field inert provider snapshot and route through resourceful ordinary/live-inspection callbacks. The Runner drains every already-admitted sibling before processing terminal completions, releases I8B capacity only when `capacitySafe` is true, retains UNSAFE reservations, preserves guest CaseRuns that were actually produced, records per-attempt infrastructure completions, never fabricates a guest result when provisioning prevented guest start, prevents same-window rescan and all later fixed-H windows after infrastructure cutover, including later resource-free cases, and records exact selected CaseSpecs that were never admitted. The returned D108 envelope is private preservation state rather than final reporting policy; public Main and production provider-registry/reporting wiring remain unchanged. Add focal SAFE, UNSAFE and healthy mixed scheduler proofs. Implementation version becomes `0.2.455-SNAPSHOT`.

## 0.2.454-SNAPSHOT
- Ratify `PLAT030 — C-prime ownership for asynchronous I/O lifecycle release/close` (GitHub #436) as Candidate A′ after explicit project-owner approval and exhaustive current/historical Truffle review including Apple Pkl, GraalJS, SOMns, GraalPy, grCUDA, Yona, TruffleSqueak, Espresso, TruffleRuby and PorcE/Orc plus secondary non-Truffle evidence. Select one private lifecycle-release-owned C-prime execution record per logical close, scheduled/resumed only by the owning ActorExecutionDomain; keep repeated close Futures as equal outcome followers of the lifecycle and keep ProtosIoLifecycle as cutover/drain/shared-outcome/follower authority rather than a generic continuation interpreter. Reject synthetic ordinary ProtosIoOperation identity, Future-owned continuation state, hidden Task topology, generic Actor-job identity introduced solely for release, Java/native continuation state machines, parked carriers and replay. Require a new PLAT gate if release execution would acquire Task-like parent/child, detach, result-Future, guest-visible identity, generic job submission or independent cancellation semantics. Governance only: no specification, executable implementation, public API, native boundary or Maven implementation-version change.

- Close `LIB011-A` through `LIB011-A3 — adversarial model closure` and reconcile A1/A2 coverage with the repository Protos-first testing policy. Add the authoritative `protos/tests/library/cli/**` semantic corpus plus the thin `ProtosCommandLineModuleTest` execution harness, while retaining the A1/A2 JUnit tests as complementary shape/implementation evidence. Cover canonical spec construction, invalid descriptors, deep snapshot/freeze, recursive lossless result data, direct token provenance, literal versus structural `--`, attached values, shared short-cluster indices, duplicate spellings/keys and recursive descriptor cycles. Production `CommandLine.protos`, public API, specification, parser behavior, native boundary and implementation version remain unchanged; `LIB011-B` becomes READY after the deterministic LIB011 affected-test closure passes. Broader integrated validation remains deferred to the owning top-level LIB011 closure.

- Implement `LM009-G4-A2 — standard LSP go-to-definition wiring` over the published G4-A1 D110 proof core. Advertise `textDocument/definition`, accept requests only for exact current open-document URIs simultaneously owned by exactly one current canonical ProjectBinding source, convert LSP UTF-16 positions to parser offsets, and revalidate both snapshot freshness and ProjectBinding authority before publication. Map only the current same-snapshot complete proof set to standard LSP Locations; loose or noncanonical documents, invalid positions, lost authority, cross-document targets not yet wired by this generation, and every unproven D110 case return an ordinary empty result. No workspace-symbol fallback, filesystem guessing, runtime assistance, guest execution, new definition semantics or specification change is introduced. Implementation version becomes `0.2.454-SNAPSHOT`.

## 0.2.453-SNAPSHOT

- Migrate standard `TextWriter.writeText`, `writeLine`, and `flush` deferred target callbacks to the ratified PLAT029 operation-owned C-prime path under `PERF006-B`. Preserve the existing ordered Java queue, transactional per-flow encoder state, empty-write zero-transition rule, commitment/cancellation boundary, exact downstream failed-Future Error identity, cancelled/invalid lower-Future to `IOError` mapping, permanent output poisoning, and later-operation gating. The Bytecode path now invokes arbitrary target `write`/`flush` guest behavior inside a Context-local synthetic C-prime root, may suspend there without replay, and awaits the returned Future through the published non-Task retain/ready/release handshake; backend Future completion only schedules the owning Actor-domain operation. Direct/AST invocation retains the historical native Java state-machine path as staged compatibility. `TextWriter.close` finalization/owned-target release remains outside this slice. No public semantic or specification change. Implementation version becomes `0.2.453-SNAPSHOT`.

## 0.2.452-SNAPSHOT

- Implement `LM009-G4-A1 — generation-1 exact static definition proof core` under ratified D110 B-prime. Add an editor-neutral complete-proof carrier and parser-authoritative intraprocedural resolver for same-activation Closure parameters plus singleton-proven match Binder/Alias origins. Preserve exact left-to-right parameter-default installation, fail closed after opaque guest invocation, treat `&&`/`||` RHS names as deferred outer captures because the RHS lowers to a lazy Closure, keep assignment/creation targets themselves unsupported while preserving an existing binding origin across pure writes, and decline outer captures, capture-interface names, multi-origin OR, module, receiver/member/delegation and runtime-assisted cases. Add session snapshot freshness support and focused positive/negative proofs; no LSP `definitionProvider`, workspace-symbol fallback, guest execution, specification change or runtime execution-path overhead is introduced. Implementation version becomes `0.2.452-SNAPSHOT`.

## 0.2.451-SNAPSHOT

- Implement `TOOL002-I8D4C2 — resourceful live-result inspection parity`. Extend the private I8D4B resourceful attempt bridge with an inspection request that reuses the same transactional provider provisioning, frozen RootActor `resources` projection, explicit off-domain submission and D108 terminalization. Source evaluation, cooperative drain-to-idle and inspector execution remain inside the same fresh resourceful Process; the inspector receives the still-live source result as a real RootActor-local Task, so Future/live-object expectations do not cross the detached boundary prematurely. Add private `resourceExecutionInspectAsync(source, inspector, bindings)` caller-domain rematerialization with the same guest-observation/infrastructure/capacity envelope as C1. Guest source/inspector Error remains guest evidence; host/provider/cleanup failure remains infrastructure evidence. Add focal proofs for a resource-derived live Future, inspector guest failure, D108 cleanup, and malformed binding input. Runner/Main, I8B release, cancellation policy and round-drain/fail-stop remain unchanged. Implementation version becomes `0.2.451-SNAPSHOT`.

## 0.2.450-SNAPSHOT
- Implement `LIB011-A1 — command-line specification model`. Publish ordinary `std:cli/CommandLine` constructors `option(descriptor)`, `positional(descriptor)`, and `command(descriptor)` for the ratified E-prime model. Constructors validate the approved logical-key, portable ASCII external-name, baseline cardinality, per-command uniqueness, positional ordering and nested-command graph invariants; canonicalize into fresh tagged structural records; snapshot every retained option/positional/subcommand Array; and freeze every retained Array and specification node. Nested command construction rejects recursive descriptor cycles and never retains caller descriptors or caller-owned aggregate Arrays. No token parser, ParseResult/result model, help renderer, typed decoder/default layer, command dispatch, environment/config/TTY authority, public parse-error taxonomy, or host/native parser bridge is added. Validation is focal-bounded to `ProtosCommandLineSpecModuleTest`; integrated LIB011 model closure remains LIB011-A3. Implementation version becomes `0.2.450-SNAPSHOT`.

## 0.2.449-SNAPSHOT

- Wire standard `Future.value()` into the PLAT029 operation-owned C-prime substrate for non-task-backed asynchronous I/O execution. Keep the existing Task-backed PLAT019 path unchanged. A private Future observer/dependency handshake now separates atomic Future terminal observation from Actor-domain operation scheduling: terminalization before C-prime retention is consumed synchronously, terminalization after retention only marks the owning operation runnable, and operation terminality releases the exact observer without cancelling the observed Future. Add an explicit retain/ready/release hook to the private operation suspension leaf so ready-before-retain and terminal-after-retain races cannot lose a wakeup or execute guest code on a Future/backend completion thread. Preserve exact failed-Future Error identity and keep `liveTaskCount()` at zero throughout operation-owned waits. No TextWriter/TextReader/buffered-I/O migration yet; no public semantic change. Implementation version becomes `0.2.449-SNAPSHOT`.

## 0.2.448-SNAPSHOT

- Implement `TOOL002-I8D4C1 — D108 resourceful caller-domain Protos rematerialization`. Add a private bootstrap-local `resourceExecutionAsync` facility over the closed I8D4B host bridge. The facility accepts exact source plus a non-empty inert snapshot of already-bound/already-reserved D077/D097/D098 facts, returns one ordinary caller-domain Future, and rematerializes only after the I8D4B terminal attempt has completed. The frozen completion object preserves the existing detached guest observation (outcome plus stdout/stderr) or null, ordered infrastructure failures as inert diagnostic strings, `infrastructureFailed`, and independent `capacitySafe`. Infrastructure/provider/cleanup failure is therefore resolved as data rather than guest Error; clean provisioning rollback may be infrastructure-failed yet SAFE, while failed terminal/provider cleanup is UNSAFE. Add focal proofs for successful RootActor resource projection, safe provisioning failure, unsafe cleanup failure and malformed private binding input. No live-result inspection variant, Runner/Main change, I8B release or D108 round-drain/fail-stop is included. Implementation version becomes `0.2.448-SNAPSHOT`.

## 0.2.447-SNAPSHOT
- Implement `I038-D7C — Bytecode Map matching parity` (GitHub #391). Extend production CanonicalMatch Bytecode lowering with D086 normal-standard-Map keyed patterns and D095 query-key timing: one stable shallow association snapshot before any query-key expression/hash/equality behavior, left-to-right once-only query evaluation, normal recorded-hash plus query-key `==` search in snapshot insertion order, all key resolution before mapped-value children, repeated requirements with once-only residue accounting, exact residue rejection before child effects, D083 capture composition, and fresh frozen standard-Map remainder materialization preserving representative key/value references, recorded hashes and insertion order without guest reinsertion. Query-key expressions, `hash`, `==` and child matchers execute through the existing C-prime continuation machinery; focal evidence suspends all four stages in sequence, mutates the live Map after snapshot acquisition, and confirms no replay plus stable selected value/remainder semantics. Structural Array and Map production Bytecode parity are now both present; integrated full-suite validation and I038 closure remain deferred to I038-D7D. This intermediate publication uses `FOCAL_BOUNDED`. Implementation version becomes `0.2.447-SNAPSHOT`.

## 0.2.446-SNAPSHOT

- Close the bounded `LM009-G4` generation-1 implementation audit under ratified `D110` without changing the definition contract. Classify same-activation Closure parameters and singleton-proven match Binder/Alias origins as the first exact proof domain, but require conservative invalidation across opaque effects because execution contexts are ordinary mutable objects. Defer outer lexical captures, local slot/assignment flow, per-name opaque capture-interface provenance, multi-origin OR, modules and receiver/delegation cases until their complete proof is available. Release `LM009-G4-A1` for the editor-neutral proof core and keep A2 LSP wiring blocked on A1. Governance/implementation-audit only: no specification, executable implementation, tests or Maven implementation-version change.

- Implement the second executable `PLAT029` foundation under `PERF006-B`: add the real operation-owned Bytecode DSL C-prime driver for non-task-backed asynchronous I/O work. `ProtosIoOperation` now owns a fresh private non-Task execution activation and lazy dynamic-control state while preserving the origin Actor/module/Prelude authority; that ownership propagates through ordinary Bytecode activation inheritance without manufacturing Task identity. Add an operation-specific suspension leaf and dependency contract distinct from `ProtosTask.WaitDependency`; the driver retains the complete top-level `ContinuationResult` only at genuine suspension, closes ready-before-publication races, resumes exclusively through the owning Actor domain, releases the exact wait relationship once, and drops retained continuation/dependency state when operation lifecycle terminality (including pre-commit cancellation or close cutover) wins. Preserve the existing rule that non-task-backed `Future.value()` waits under its enclosing lifecycle and has no Task cancellation flag. This slice deliberately does not yet change standard `Future.value()` or migrate TextWriter/TextReader/buffered I/O. No public Protos-visible semantics change. Implementation version becomes `0.2.446-SNAPSHOT`.

## 0.2.445-SNAPSHOT

- Implement the first executable `PLAT029` foundation under `PERF006-B`: add one private operation-owned deferred C-prime driver slot and Actor-domain runnable/resume seam for non-task-backed `ProtosIoOperation` work. Readiness publication only queues the operation; arbitrary backend/completion threads never execute operation guest work inline. Tasks and operation segments share the Actor-local FIFO implementation queue without creating hidden Tasks, Task parentage, detach semantics or a second scheduler. Repeated readiness while queued coalesces, one readiness request while running produces exactly one later segment, and terminal queued operations are skipped. This slice deliberately does not yet store a Truffle `ContinuationResult`, change `Future.value()`, or migrate TextWriter/TextReader/buffered I/O; those remain the next PLAT029 slices. No public Protos-visible semantics change. Implementation version becomes `0.2.445-SNAPSHOT`.

## 0.2.444-SNAPSHOT

- Close `LIB010-C` through `LIB010-C2 — encoder hardening/conformance`. Replace recursive semantic Array/Table `visit` descent in `TOML.encode` with invocation-local explicit frames for the root table, Arrays and inline Tables while retaining identity-path cycle detection and stable Map association order. Add focused encode→parse stress evidence for 2,048 nested Arrays and 1,024 nested Tables, confirm shared acyclic containers do not false-positive as cycles, and retain deterministic temporal spelling including D104 `second = 60` and zero-offset `Z`. All ten semantic kinds and D109 shortest-round-trip Float behavior remain unchanged; no Java/native boundary, host formatter, D087 tool TOML, public resource limit, streaming, Document or I/O authority change is introduced. `LIB010-D` is READY for integrated conformance/stress/isolation and top-level baseline closure. Implementation version becomes `0.2.444-SNAPSHOT`.

## 0.2.443-SNAPSHOT

- Implement `TOOL002-I8D4B — D108 resourceful async attempt bridge`. Correct the I8D4A guest lane to preserve the complete existing `ProtosCapturedProcessExecution.Result` including captured stdout/stderr. Add a private `ProtosTestResourcefulAttemptBridge` that accepts one non-empty already-reserved binding set, performs I8D2 transactional provisioning, then submits only successfully provisioned child execution through the explicit off-domain Test Tool submission boundary. Build the frozen I8D3 `resources` Map, bootstrap and execute one fresh Process, capture the ordinary detached execution input, then complete only after I8D4A semantic Process + execution-host terminality and D107 ProviderLease cleanup. Preserve provisioning failure without fabricating guest evidence; clean rollback yields infrastructure-failed but capacity SAFE, while rollback cleanup failure remains ordered secondary evidence and yields UNSAFE. Submission/bundle/bootstrap failure after successful provisioning likewise cleans the provider transaction. Infrastructure-failure presence and capacity safety are independent. No I8B release, Protos-side Future/rematerialization, Runner round-drain or public Main wiring is included. Implementation version becomes `0.2.443-SNAPSHOT`.

## 0.2.442-SNAPSHOT
- Ratify `D110 — Static go-to-definition proof boundary, identity and ambiguity contract` (GitHub #435) as Candidate B′ after explicit project-owner approval and exhaustive comparison across Self, Pharo/Smalltalk, TypeScript, Pyright/Jedi, Ruby tooling, LuaLS, ElixirSense, clojure-lsp, Erlang ELP, rust-analyzer, clangd, SourceKit-LSP, JDT LS and LSP itself. Define `textDocument/definition` as a complete finite static proof set: one exact source target for a proven singleton, all targets only when the finite set is proven complete, and no result when completeness is unknown. Forbid workspace-symbol/name/proximity/path heuristics, semantic result truncation, filesystem guessing and runtime/guest assistance; allow generation 1 to be singleton-first/mechanically exact and expand monotonically as analysis improves. Governance/tooling only: no specification, executable implementation or Maven implementation-version change; LM009-G4 becomes READY_FOR_IMPLEMENTATION_AUDIT.

- Implement `I038-D7B — Bytecode Array matching parity` (GitHub #391). Extend production CanonicalMatch Bytecode lowering with D084 standard-Array structural patterns: exact fixed shape, one optional middle remainder, pre-child shallow indexed observation, deterministic prefix/remainder/suffix child order, D083 capture composition, effect retention on later mismatch, nested Array/value/OR/alias composition, and fresh frozen standard-Array remainder materialization. The Bytecode root stores only inert Array-attempt snapshot state; all child matchers remain in the existing C-prime continuation chain, including suspension/resume without replay. Focal evidence mutates the original Array while a prefix matcher is suspended and confirms later remainder/suffix matching still observes the pre-child snapshot. Map matching and D086 stable-snapshot semantics remain explicitly deferred to I038-D7C. This intermediate publication uses `FOCAL_BOUNDED`; integrated full-suite validation remains debt of top-level I038/D7D. Implementation version becomes `0.2.442-SNAPSHOT`.

## 0.2.441-SNAPSHOT

- Implement `LIB010-C1 — functional TOML semantic encoder`. Publish `TOML.encode(rootTable)` for all ten ratified TOML semantic kinds using deterministic TOML 1.1 output: quoted basic-string keys, inline tables for nested semantic tables, ordinary arrays (including arrays of tables), exact unbounded Integer decimal output, canonical temporal spellings with D104 `second = 60`, and D109-C host-independent shortest-round-trip binary64 Float rendering (`0.0`, `-0.0`, `inf`, `-inf`, `nan`). Implement D109 entirely in ordinary Protos using exact Integer arithmetic and existing numeric conversions; add no Java/native helper or host display dependency. Add focal semantic round-trip, escaping, cycle/fail-closed and hard binary64 evidence. C2 remains READY for iterative deep-container hardening and encoder conformance closure. Full integrated validation remains debt of top-level LIB010. Implementation version becomes `0.2.441-SNAPSHOT`.

## 0.2.440-SNAPSHOT

- Implement `I038-D7A — Bytecode non-structural matching parity` (GitHub #391). Add direct CanonicalMatch lowering to the production Bytecode backend for value, binder, wildcard, alias and ordered OR patterns, including D088 ordinary positional capture ABI, D089 first-success OR commitment, D096 matcher tri-state decoding, guarded-arm continuation, fresh terminal no-match Error, invalid-outcome Error and selected-arm arity no-retry behavior. Matcher, guard and arm-body guest execution is emitted exclusively through the existing prepared C-prime invocation machinery, so suspension/resume remains inside the Bytecode continuation chain with no Java helper acting as a hidden guest continuation; the focal includes sequential matcher/guard/body suspension with exactly-once replay evidence. Structural Array parity remains deferred to I038-D7B and D086 stable-snapshot Map parity remains deferred to I038-D7C. This intermediate publication uses `FOCAL_BOUNDED`; integrated full-suite validation remains debt of top-level I038/D7D. Implementation version becomes `0.2.440-SNAPSHOT`.

## 0.2.439-SNAPSHOT

- Ratify `PLAT029 — Deferred C-prime ownership for non-task-backed asynchronous I/O operations` (GitHub #433) as Candidate B′ after explicit project-owner approval and exhaustive current/historical Truffle review including Apple Pkl, GraalJS, GraalPy, SOMns, TruffleSqueak, Espresso, TruffleRuby and PorcE/Orc plus mature non-Truffle async runtimes. Select operation-owned private C-prime continuation custody with ActorExecutionDomain scheduling/resume while keeping the non-task-backed result Future strictly outcome/observation/cancellation-signalling state. Forbid backend-thread guest re-entry, hidden `Future.then`/Task topology, parked carriers, Java-stack continuation, replay islands and a generic second host continuation stack; require a new PLAT gate if the minimal non-Task driver would acquire Task-like parent/child ownership, detach or observable execution identity. Release bounded PERF006-B TextWriter, TextReader and buffered-I/O migration. Governance only: no specification, executable implementation, public API, native boundary or Maven implementation-version change.

- Ratify `D109 — TOML binary64 float encoding and portable decimal-rendering contract` (GitHub #434) as Candidate C after explicit project-owner approval and comparative audit across TOML/toml-test, Rust, Go, Python, .NET, C++, Java, JavaScript, Ruby and Ryū/Schubfach-class algorithms. Define host-independent canonical shortest-round-trip binary64 rendering with TOML normalization: `0.0`, `-0.0`, `inf`, `-inf`, canonical `nan`, lowercase scientific notation when selected, and `.0` for otherwise integral-looking finite Float tokens. Keep source lexemes in the future Document layer, do not make JVM `Double.toString` normative, and require a new Dxxx/PLATxxx gate if implementation needs a privileged runtime/bit-decomposition capability. Governance/documentation only: no executable implementation, specification, D087 behavior, native boundary or Maven implementation-version change; LIB010-C encoder implementation is READY.

- Implement `LM009-G3 — canonical workspace symbols` (GitHub #360) under ratified D079/D082/D085/D089/D102/D106. Advertise standard LSP `workspace/symbol`; treat file-backed workspace folders/rootUri only as exact ProjectBinding acquisition candidates; maintain session-local per-project incremental source indexes over canonical workspace-owned sources; overlay only the exact current open canonical document; exclude loose documents, dependencies and `std:`; and fail closed on invalid project/source/parse state without project discovery or guest execution. Apply D106 C′ exactly with NFC + locale-independent Unicode Default Case Folding, ordered-code-point subsequence eligibility, exact/prefix/substring/subsequence ranking, case-exact tie preference, canonical deterministic ordering and one global Top-100 heap. G4 definition semantics remain unimplemented and decision-gated. This intermediate publication uses `FOCAL_BOUNDED`; integrated/full and real VS Code S5 validation remain debt of LM009-G/#360. Implementation version becomes `0.2.439-SNAPSHOT`.

## 0.2.438-SNAPSHOT

- Implement `TOOL002-I8D4A — D108 terminal attempt envelope foundation`. Add a host-only `ProtosProcessRuntime.awaitTerminationForRuntime()` barrier over the actual TERMINATED transition; extend `ProtosProcessExecutionHost` with a terminal-disposition wait and make the Polyglot host distinguish Context close request from successful/failed terminal close. Add private `ProtosTestResourceAttemptCompletion` with separate optional guest observation, ordered infrastructure evidence and SAFE/UNSAFE capacity disposition, plus `ProtosTestResourceAttemptTerminalizer` which preserves guest evidence, waits Process and host terminality, then awaits D107 ProviderLease cleanup. Host/Context or provider-cleanup failure is infrastructure evidence and marks capacity UNSAFE; provider cleanup is still attempted after host terminal failure and no failure is converted to guest Error. No provisioning-failure bridge, I8B release, Runner round-drain or public Main wiring is included. Implementation version becomes `0.2.438-SNAPSHOT`.

## 0.2.437-SNAPSHOT
- Implement `PERF006 PLAT028 Map.remove C-prime orchestration` for the exact standard `Map.remove` method. Preserve mutation-entry and open-state preflight before key callbacks, one query `hash` call, recorded-hash filtering, `queryKey == storedKey` equality direction, canonical-Boolean validation, exact removed-value identity, missing-key Error behavior and the post-callback open-state recheck before removal. Keep each Map comparison scope active across callback suspension and release it exactly once on normal completion, Error, non-local control or cancellation; same-Map keyed mutation therefore fails while a comparison callback is suspended. Retain Java `remove/find/hash` solely as staged AST/native compatibility. `Map.at`, `containsKey` and `atPut` C-prime paths remain unchanged. No public API or Protos-visible semantics change. Implementation version becomes `0.2.437-SNAPSHOT`.

## 0.2.436-SNAPSHOT

- Implement `I038-D6 — D096 conservative match coverage analysis` (GitHub #391). Add a compiler-only resource-bounded tri-state analyzer over the ratified D095 surface pattern algebra and attach its result to `CanonicalMatch` during canonicalization while source structure is still available. Unguarded syntactically universal `_`/binder/alias/group/ordered-OR forms prove `PROVEN_EXHAUSTIVE`; matches composed solely from standard Array/Map structural patterns prove `PROVEN_NON_EXHAUSTIVE` using the sound witness class `NON_ARRAY_OR_MAP_VALUE`; ordinary matcher/value forms and guards remain coverage-opaque and therefore produce `UNKNOWN` when no independent proof exists. Exhausted analysis budget fails closed to `UNKNOWN` and is recorded explicitly. Existing parser-owned D096 structural invalidity rules remain the only coverage-related Core source errors. The proof is compiler IR metadata only: no runtime Pattern/CoverageSignature hierarchy is introduced, runtime matching order/effects are unchanged, and D092 terminal fresh-Error fallback is deliberately retained even for `PROVEN_EXHAUSTIVE` in this slice. Richer warning/lint usefulness analysis and any no-match-branch optimization remain optional future work. This intermediate publication uses `FOCAL_BOUNDED`; full integrated validation remains debt of top-level I038 closure. Implementation version becomes `0.2.436-SNAPSHOT`.

## 0.2.435-SNAPSHOT

- Close `LIB010-B` through `LIB010-B2B2B — numeric/AoT scaling + parser closure`. Make Integer, Float and temporal-fraction token scans iterative; remove host recursion from nested-value and document balanced chunk helpers; replace repeated whole-Array rebuilding for `[[array-of-tables]]` with invocation-local balanced builders materialized once after parsing; retain focused stress evidence for 2,048-digit numeric/temporal tokens and 2,048 repeated array-of-tables alongside prior B2 evidence; and leave only grammar-bounded helper recursion. No public resource limit, D087/private tool TOML, encoder, Document, streaming or Java/native boundary changes. Integrated full validation remains debt of top-level LIB010; LIB010-C Float-formatting audit is READY. Implementation version becomes `0.2.435-SNAPSHOT`.

## 0.2.434-SNAPSHOT

- Ratify `D108 — Test Tool infrastructure outcome propagation, terminal cleanup barrier and unsafe-capacity failure policy` (GitHub #427) as Candidate C′ after explicit project-owner approval and comparative audit across Kubernetes DRA/device health, Nomad task/device lifecycle, Slurm prolog/epilog drain behavior, Bazel/remote execution, Buck2 local resources, pytest, JUnit, Go testing cleanup, Terraform provider diagnostics, Testcontainers/Ryuk, CI runner semantics, structured concurrency and WASI resource ownership. Define one terminally-closed private attempt completion with separate guest-observation and infrastructure-outcome lanes; resourceful attempt completion occurs only after provisioning/rollback disposition, guest execution if started, real Process and execution-host/Context terminalization, reverse-order all-attempted ProviderLease cleanup and safe-capacity disposition. Infrastructure failure never becomes guest `Error`. Release I8B reservation capacity only when reuse is confirmed safe; retain unsafe/unconfirmed capacity. If any admitted attempt has infrastructure failure, drain every already-admitted sibling to its own terminal boundary, release only safe reservations, then fail-stop the entire invocation before D105 rescan/new-window admission, without continuing later resource-free tests in the baseline. Preserve already-produced guest evidence and fabricate no test failure for provisioning failure or cases never started by the infrastructure cutover. Fault-domain quarantine/continuation and recovery remain deferred. Governance/tooling only: no executable implementation, specification, native-boundary or Maven implementation-version change.

- Implement `I038-D5 — guarded arm execution and D092 no-selection semantics` (GitHub #391). Guard and body share the already-existing ordinary D088 synthetic arm Closure activation, so the exact same bindings are visible to both, guard-side binder mutation reaches the body, and captured non-local-return home survives plan rebuild/projection. Canonical `true` executes the body; canonical `false` rejects only the current arm; any other normal guard result signals ordinary generic `Error`. A private Java-only rejection sentinel is consumed immediately by `ProtosMatchNode`; it is not guest-visible and changes neither D072 nor D088. Pattern mismatch skips the guard, false-guard effects remain visible, D090 first-success OR is never reopened, Error/control/cancellation/suspension do not retry, and terminal no-selection remains a fresh ordinary Error. Bytecode DSL matching and D096 coverage optimization remain deferred. Full integrated validation remains debt of I038. Implementation version becomes `0.2.434-SNAPSHOT`.

## 0.2.433-SNAPSHOT

- Implement `PERF006 PLAT028 Map.atPut C-prime orchestration`. The exact standard `Map.atPut` now keeps its full query-key `hash` and `queryKey == storedKey` search sequence in Bytecode C-prime, with Map comparison scopes surviving suspension and unwinding exactly once. Preserve I035 semantics: mutation/frozen guard before callbacks, one exact Integer-family query hash per operation, post-hash stable entry snapshot, recorded-hash filtering in insertion order, query-side canonical-Boolean equality, closed-Map replacement of an existing representative, original representative-key/order retention, absent insertion only while open, and reuse of the exact search hash as the inserted entry's recorded hash. Cancellation/error/non-local unwind performs no delayed mutation. The Java `atPut/find/hash` implementation remains staged AST/native compatibility; `Map.remove` is deliberately excluded. No public API or guest-visible semantic change. Implementation version becomes `0.2.433-SNAPSHOT`.

## 0.2.432-SNAPSHOT

- Implement `LIB010-B2B2A — TOML textual lexical scaling` as an intermediate `FOCAL_BOUNDED` parser-hardening slice. Replace recursion proportional to input length in quoted/bare keys, basic/literal Strings, whitespace/trivia and bare-token gather/trim/copy paths with iterative state. Retained stress evidence adds 4,096-character keys and 8,192-character Strings/whitespace while retaining B2A/B2B1 evidence. B2B2B remains READY for numeric token loops, array-of-tables scaling and final LIB010-B closure. No public resource limit, semantic/API, D087, encoder, Document, streaming or Java/native boundary change is introduced. Full integrated validation remains debt of top-level LIB010. Implementation version becomes `0.2.432-SNAPSHOT`.

## 0.2.431-SNAPSHOT

- Implement `PERF006 PLAT028 Map read-lookup C-prime orchestration` for the exact standard `Map.at` and `Map.containsKey` methods. Preserve one query `hash` call, exact Integer-family hash value, insertion-order recorded-hash filtering, `queryKey == storedKey` direction, canonical-Boolean equality validation and exact `at` value identity. Keep each Map comparison scope active across callback suspension and release it exactly once on normal completion, Error, non-local control or cancellation through Bytecode `try/finally`; conflicting same-Map keyed mutation therefore continues to fail without blocking while unrelated Actor work remains schedulable. Retain the Java `find/hash` path solely as staged AST/native compatibility. `atPut` and `remove` mutation/search orchestration are deliberately deferred to the next bounded slice. No public API or Protos-visible semantics change. Implementation version becomes `0.2.431-SNAPSHOT`.

## 0.2.430-SNAPSHOT

- Implement `LIB010-B2B1 — TOML document/path scaling` as an intermediate `FOCAL_BOUNDED` parser-hardening slice. Replace quadratic document statement Array rebuilding with balanced chunk accumulation; make dotted-key path construction and document dotted/header walks iterative; and remove recursive assignment/trim/path scanning from that path. Retained stress evidence covers 4,096 flat assignments and 1,536-component dotted assignment/header paths, while retaining the B2A deep nested-value tests. No public/tunable resource limit, semantic/API change, encoder, Document, streaming surface, D087 change or Java/native boundary change is introduced. B2B2 remains READY for remaining scalar/token recursion and repeated array-of-tables accumulation before LIB010-B closes. Full integrated validation remains debt of top-level LIB010. Implementation version becomes `0.2.430-SNAPSHOT`.

## 0.2.429-SNAPSHOT

- Implement `I038-D4 — ordered OR / alternative-pattern execution` (GitHub #391) under ratified D072/D073/D083/D088/D090/D095/D103. Add a private production `OrPatternNode` that attempts alternatives exactly once in semantic left-to-right order against the same ordinary subject, continues only on ordinary mismatch, returns immediately on the first successful existing capture sequence, retains effects of failed alternatives, and propagates Error/control/suspension without retry. Lower canonical OR patterns recursively without a branch tag, rollback state, speculative execution, capture registry or second matcher authority. Close the source/static compatibility hole for dynamic opaque capture interfaces: OR branches now compare the complete representable logical D088/D103 interface (`required names + optional terminal rest name/rest-ness`), while compatible `captures(first, ...rest)` alternatives remain ordinary and lower to one Closure fixed-prefix + terminal rest ABI. The lowerer defensively verifies the same interface if canonical IR bypasses the parser. Guards, D096 coverage optimization and Bytecode DSL matching remain later I038 slices. This intermediate publication uses `FOCAL_BOUNDED`; full integrated validation remains debt of top-level I038 closure. Implementation version becomes `0.2.429-SNAPSHOT`.

## 0.2.428-SNAPSHOT

- Implement `TOOL002-I8D3 — D107 RootActor resource capability-bundle projection`. Build the guest resource bundle as the ordinary Core `Map`, inserting exact D077 String keys via the standard `Map.atPut` protocol so canonical String hash/equality semantics remain language-owned, then freeze the Map before bootstrap. Extend `ProtosProcessRuntime` with one bootstrap-stable optional frozen resource-Map grant parallel to existing Filesystem/Network grants; `ProtosActorBootstrap` validates frozen state and exact current-Prelude Map parent before exposing that grant only as the RootActor initial local `resources`. Keep all existing public standalone bootstrap signatures unchanged and add only a package-private resourceful entry accepting the already-built Map. Focal guest execution proves `resources["gpu"]` returns the exact provisioned value; Java-side evidence proves Map immutability, canonical lookup, resource-free absence and fail-closed rejection of unfrozen/foreign-Prelude bundles. No ProviderAdapter/ProviderLease/registry authority enters guest state, and no I8D2-to-Runner/Main or I8B-release integration occurs yet. Implementation version becomes `0.2.428-SNAPSHOT`.

## 0.2.427-SNAPSHOT
- Implement `TOOL002-I8D2 — D107 multi-provider transaction coordinator`. Add a private `ProtosTestResourceProviderCoordinator` over the I8D1 exact registry/request/adapter/lease foundation and a host-owned successful `ProtosTestResourceProviderTransaction`. Snapshot already-reserved binding facts, reject duplicate resource keys before any provider call, group bindings once per logical provider in stable first-appearance order and provision those groups sequentially through async-capable adapter stages. Require every lease to cover exactly its provider group's D077 keys before continuing. On unresolved provider, provisioning failure or invalid lease coverage, clean the current and prior successful leases in strict reverse acquisition order, attempt every cleanup even after one fails, preserve the primary failure and attach later cleanup failures as infrastructure evidence. On success, retain all leases host-side and expose only an immutable candidate-capability snapshot restored to original requirement-key order; successful transaction cleanup follows the same reverse-order/all-attempted rule. Empty resource input performs zero provider work. Process/RootActor `resources` projection, Runner/Main wiring and I8B reservation release remain absent. Implementation version becomes `0.2.427-SNAPSHOT`.

## 0.2.426-SNAPSHOT
- Implement `TOOL002-I8D1 — private D107 provider foundation`. Add an immutable exact environment `ProtosTestResourceProviderRegistry` with explicit duplicate rejection and no ambient discovery/fallback; a transport-neutral host `ProtosTestResourceProviderAdapter` boundary whose provisioning result is `CompletionStage`-capable; an immutable one-provider `ProtosTestResourceProviderRequest` that snapshots the already-ratified inert resource-key/mode/units/scope/provider/profile facts without reinterpreting them; and a host-owned `ProtosTestResourceProviderLease` that snapshots candidate guest capability values while retaining asynchronous cleanup machinery outside guest state. Focal evidence proves exact case-sensitive provider resolution, duplicate fail-closed behavior, single-provider request isolation, immutable snapshots, deferred asynchronous provisioning/cleanup and the absence of Runner/Main/RootActor `resources` wiring. No I8A binding grouping, multi-provider transaction/rollback, required-key lease validation, guest bundle projection, Process bootstrap change or I8B release occurs yet. Implementation version becomes `0.2.426-SNAPSHOT`.

## 0.2.425-SNAPSHOT
- Ratify `D107 — Test Tool provider resolution, provisioning transaction and attempt-private capability delivery contract` (GitHub #426) as Candidate D′ after explicit project-owner approval and comparative audit across Kubernetes DRA/device plugins/CDI, Nomad, Terraform/go-plugin, Buck2 local resources, CTest, Bazel, pytest, JUnit lifecycle resources, WASI/WebAssembly Component Model, Testcontainers and OCI/CDI projection. Resolve inert D098 provider identities only through an exact environment-owned registry into transport-neutral configured ProviderAdapters; permit an in-process implementation without making Java the durable ABI. Group each attempt's already-reserved bindings per provider and provision all provider groups transactionally before child Process creation; unwind partial success in reverse acquisition order. Keep successful ProviderLeases host-owned through terminal child cleanup, then clean providers in reverse order before releasing I8B scheduler capacity. Project one frozen RootActor-only `resources` bundle keyed by exact D077 resource key, with one guest capability value per required key and no provider/registry/credential/cleanup authority in guest state. Resource-free attempts keep the zero-provider fast path and no `resources` local. Unknown provider/profile, provisioning and cleanup failures remain infrastructure outcomes; cleanup that cannot be confirmed safe must not silently return capacity. Governance/tooling only: no executable implementation, specification, native-boundary or Maven implementation-version change.

- Implement `I038-D3 — standard Map matching execution` (GitHub #391) under ratified D083/D086/D088/D095/D103. Add a private Map pattern node to the production AST matching path. Each attempt accepts only normal standard `ProtosMapValue`, captures one stable shallow association snapshot before query-key behavior, evaluates query-key expressions once left-to-right, computes each query hash once, resolves all requirements against the same snapshot with the existing normal Map recorded-hash + query-side `==` relation, fixes every selected mapped-value reference before child matching, and then executes mapped-value children in source requirement order. Repeated/equivalent requirements may select the same association and residue accounting selects that association once. Open/subset remains the default; `exact` validates empty residue before child execution. A nested remainder executes after successful mapped-value children and receives one fresh frozen standard Map reconstructed internally from unselected snapshot associations while preserving representative keys, mapped-value references, recorded hashes and insertion order without ordinary `hash`, `==`, `atPut` or iteration sends. Refactor `ProtosStandardMapProtocol` only enough to share the exact same key-comparison core with D086 snapshot lookup; ordinary Map behavior is unchanged. OR, guards, D096 coverage optimization and Bytecode DSL matching remain later I038 slices. This intermediate publication uses `FOCAL_BOUNDED`; full integrated validation remains debt of top-level I038 closure. Implementation version becomes `0.2.425-SNAPSHOT`.

## 0.2.424-SNAPSHOT

- Ratify `D106 — Workspace-symbol query matching, ranking, ordering and result-cap contract` (GitHub #425) as Candidate C′ after explicit project-owner approval and expanded comparison across LSP 3.18/3.19, clangd, rust-analyzer, gopls, Pyright, SourceKit-LSP, Metals, Eclipse JDT LS, ZLS, Ruby LSP, HLS/HieDb, OCaml-LSP and TypeScript/tsserver. Baseline `workspace/symbol` now treats an empty query as no results; non-empty queries use NFC plus locale-independent Unicode Default Case Folding and ordered-code-point subsequence eligibility over the D079 symbol name only. Rank eligible candidates by folded exact > prefix > contiguous substring > other subsequence, then case-exact relation, shorter name and deterministic canonical identity; apply no camel/separator/path/package/project/open-document weighting. Aggregate all exact D082 project domains before one global hard Top-100 response cap. Storage/index machinery remains replaceable and no matcher/cap configuration, production implementation, core specification, native boundary or Maven implementation-version change is introduced. LM009-G3 is released for implementation.

- Implement `LIB010-B2A — iterative TOML value nesting` as an intermediate `FOCAL_BOUNDED` hardening slice. Replace recursive array/inline-table value descent with explicit invocation-local heap frames, preserve per-frame chunked array materialization, make inline-table dotted-path insertion and the document/value range copy iterative, and introduce no public/tunable nesting limit. Focused stress evidence parses and traverses 2,048 nested arrays and 1,024 nested inline tables. B2B remains READY for remaining large-flat document/path scaling before LIB010-B closes. Encoder, Document, streaming, D087 and Java/native boundary remain unchanged. Full integrated validation remains debt of top-level LIB010. Implementation version becomes `0.2.424-SNAPSHOT`.

## 0.2.423-SNAPSHOT

- Implement `LIB010-B1 — functional strict TOML 1.1 parser` as an intermediate `FOCAL_BOUNDED` slice of LIB010. Add `TOML.parse(text)` to `std:toml/TOML`, adapting the already-audited private TOML parser mechanics without introducing any runtime dependency on `tool-shared:Toml10` or changing D087. The public parser accepts TOML 1.1 basic/literal strings including `\\e` and `\\xHH`, booleans, unbounded Protos Integer, binary64 Float with signed zero/infinity/NaN, all four TOML temporal categories including omitted seconds and D104 `second = 60`, heterogeneous arrays, dotted keys, TOML 1.1 multiline/trailing-comma inline tables, tables and arrays-of-tables; malformed syntax and duplicate/redefinition/inline-table conflicts fail closed. Encoder, source-preserving Document, streaming/events and I/O authority remain absent. B2 remains READY for explicit nesting/adversarial hardening before LIB010-B closes. Full integrated validation remains debt of top-level LIB010. Implementation version becomes `0.2.423-SNAPSHOT`.

## 0.2.422-SNAPSHOT

- Implement `I038-D2 — Alias + standard Array matching execution` (GitHub #391) under ratified D083/D084/D088/D095/D103. Replace the D1 arm-kind runtime with private recursive pattern nodes for ordinary value matchers, binders, wildcard discard, whole-subject alias and standard Array patterns. Array matching accepts only real standard `ProtosArrayValue` state, establishes one shallow indexed snapshot before any child matcher, enforces exact fixed length or minimum prefix+suffix length with one remainder, executes prefix -> remainder -> suffix children once in semantic order, shallow-concatenates D072 captures, and materializes a fresh frozen standard Array only when a remainder child semantically receives the unmatched range. Alias contributes the current subsubject as one capture before its nested captures. Implement D103's newly ratified static rule in the parser: an opaque `captures(...rest)` variable segment must remain terminal in the final ordered source binding interface; later zero-binding recognition is allowed, later bindings are rejected, and structural Array/Map remainder binders remain one fixed aggregate binding. Terminal nested dynamic captures lower directly to the ordinary final Closure rest parameter; no capture repartitioning or richer carrier is introduced. Map, OR, guards, D096 coverage optimization and Bytecode DSL matching remain later I038 slices. This intermediate publication uses `FOCAL_BOUNDED`; full integrated validation remains debt of top-level I038 closure. Implementation version becomes `0.2.422-SNAPSHOT`.

## 0.2.421-SNAPSHOT

- Implement `TOOL002-I8C — D105 fixed-H-window resource-round Runner composition`. Add private `Runner.runBoundedWithResourcesWithLoader` / `runBoundedWithResources` entrypoints over the existing H scheduler, I8A static Requirement-to-CatalogEntry bindings and I8B atomic reservation kernel. Materialize one immutable logical H window of at most `jobs` selected CaseSpecs, then scan unfinished members in stable TestPlan order for complete-set reservations; admit all currently feasible members, allow only same-window backfill, wait for the admitted asynchronous execution round, release every reservation before the next scan, and retry only blocked members. Buffer completed case results by immutable window index and emit them only after the window completes so resource-round admission order cannot change deterministic TestPlan result order. Resource-free windows remain a single H round. Add a pure-Protos focal fixture proving same-window backfill (`first, third, second` admission), cross-window non-backfill (`first, second, third` with jobs=2), logical result order, and resource-free H compatibility. Public Main/I6E requirements wiring, provider resolution/provisioning, attempt-private capability delivery, placement topology and provider/lease cleanup remain deferred. Implementation version becomes `0.2.421-SNAPSHOT`.

## 0.2.420-SNAPSHOT
- Implement `PERF006 PLAT028 Map.each C-prime callback orchestration`. Give the exact standard `Map.each` implementation stable native provenance and recognize structured execution only for the canonical standard method/home selection. Establish the shallow logical association snapshot before callback #1, keep the `(key, value)` iteration cursor in C-prime, advance only after each callback truly completes, ignore callback results and return the exact Map receiver. A suspending callback therefore resumes at the exact association without replaying an already-completed prefix or retaining the Java `for` frame. Preserve the Java loop solely as the staged AST/native compatibility branch until B6B. Add retained Future suspension/resume, mutation-snapshot and anti-alias provenance evidence. Map `hash`/`==` comparison orchestration and `at`/`containsKey`/`atPut`/`remove` are deliberately not included in this slice. No public API or Protos-visible semantics change. Implementation version becomes `0.2.420-SNAPSHOT`.

## 0.2.419-SNAPSHOT

- Ratify `D105 — Test Tool resource-blocked admission ordering and backfill contract` (GitHub #424) as Candidate E′ after explicit project-owner approval and exhaustive comparison across CTest, cargo-nextest, Slurm, Kubernetes, Nomad, HTCondor, Jenkins Lockable Resources, GitLab resource groups and Buildkite concurrency groups. Preserve the existing TOOL002-H logical wave boundary as a fixed window of the next `min(jobs, remainingCases)` CaseSpecs. Within that immutable window, scan remaining members in stable TestPlan order, atomically reserve every currently feasible complete I8B resource set, leave temporarily blocked members pending inside the same window, wait for the admitted resource round to reach terminal cleanup and release, then rescan the remaining members. Never pull work from a later H window forward. Resource-free windows remain exactly the existing one-round H behavior; result/report ordering remains TestPlan order. Do not introduce global eager backfill, priority, aging, fair-share, preemption, duration prediction, placement topology or provider policy. Governance/tooling only: no executable implementation, specification, native-boundary or Maven implementation-version change. Note that this decision was initially opened with the already-allocated D102 identifier and was corrected to the next free identifier D105 before repository ratification.

- Close `LM009-G3P P4 — ProjectBinding prerequisite closure` after P1/P2/P3A/P3B and D102. Add durable closure evidence for independent candidate-root isolation, duplicate canonical physical package-root fail-closed behavior, complete generation-1 mechanical freshness invalidation, and the static no-guest/no-Package-Tool-execution acquisition boundary. Reconcile the accumulated P3 validation debt through the repository top-level publication-validation path and release LM009-G3 to READY when that closure passes. No production implementation, specification, public API, or Maven implementation-version change is included.

- Implement `PERF006 PLAT028 IdentityMap.each C-prime callback orchestration`. Give the exact standard `IdentityMap.each` implementation stable native provenance and recognize structured execution only for the canonical standard method/home selection. Establish the shallow logical association snapshot before callback #1, keep the `(key, value)` iteration cursor in C-prime, advance only after each callback truly completes, ignore callback results and return the exact IdentityMap receiver. A suspending callback therefore resumes at the exact association without replaying an already-completed prefix or retaining the Java `for` frame. Preserve the Java loop solely as the staged AST/native compatibility branch until B6B. Add retained Future suspension/resume, mutation-snapshot and anti-alias provenance evidence. Map `hash`/`==` callback orchestration is deliberately not included in this slice. No public API or Protos-visible semantics change. Implementation version becomes `0.2.419-SNAPSHOT`.

## 0.2.418-SNAPSHOT

- Implement `LM009-G3P-P3B — exact protos.project ProjectBinding provider` (GitHub #413; parent #373) under ratified D082 A′ / D085 F′ / D089 A″ / D102 A′. Add a replaceable editor-neutral ProjectBindingProvider and a generation-1 local artifact implementation that reads only the exact candidate root, rejects missing/malformed/noncanonical authority, recomputes `protos-project-metadata-v1` from exact root/member manifest plus lock bytes without parsing package semantics, binds projected package roots through the existing confinement/D102 package-domain authority, and reuses P1 source inventory to materialize the P2 immutable ProjectBinding. Ordinary source churn remains outside projection freshness; no project discovery, automatic Package Tool/guest refresh, workspace-symbol index, definition semantics or specification change is included. This intermediate P3B publication uses `FOCAL_BOUNDED`; broader integrated validation remains debt of LM009-G3P-P3 / #413. Implementation version becomes `0.2.418-SNAPSHOT`.

## 0.2.417-SNAPSHOT

- Close `LIB010-A/D104 — TOML leap-second semantic alignment` after ratified D104 Candidate B′. Extend the existing `std:toml/TOML` `localTime`, `localDateTime`, and `offsetDateTime` constructors so the ordinary Integer `second` component accepts `0..60`, preserving TOML 1.1 `:60` as format-level semantic data without consulting a leap-second schedule, tzdb, clock, host datetime type, or other ambient temporal authority. Retained focal evidence covers `second = 60` for all three time-bearing kinds and preserves fail-closed rejection of `61`. No parser, encoder, Document, streaming surface, general datetime subsystem, Java/native boundary, specification semantics, or D087 private TOOL001/TOOL002 TOML behavior changes in this slice. `LIB010-B` is released for strict TOML 1.1 parser implementation. Implementation version becomes `0.2.417-SNAPSHOT`.

## 0.2.416-SNAPSHOT

- Implement `TOOL002-I8B — private live atomic resource reservation kernel`. Add `ResourceReservation.protos` over I8A Requirement/CatalogEntry bindings. Construct one invocation-local mutable pool state from the validated catalog while keeping the state outside TestPlan data. `tryReserve` synchronously checks the complete binding set before mutation, so a contended multi-resource attempt returns null with no partial holding; `shared` consumes units up to catalog capacity and `exclusive` requires sole occupancy. Successful non-empty reservations are tracked in an `IdentityMap` by reservation identity so release is exact and single-use; release validates the complete reservation before restoring any pool, preventing a stale duplicate release from freeing a newer compatible reservation. Resource-free reservations remain frozen empty values and add no active-registry state. No Main/Runner integration, candidate ordering/fairness, placement-domain identity, provider provisioning or terminal cleanup integration is selected. Implementation version becomes `0.2.416-SNAPSHOT`.

## 0.2.415-SNAPSHOT
- Implement the shared runtime/tooling realization of ratified `D102 — nested workspace package source ownership and module-boundary contract`. Canonical workspace directory binding now rejects distinct PackageIds that materialize onto the same canonical physical root and derives descendant authorized package roots as hard source-domain boundaries. Exact workspace source lookup rejects traversal or canonical symlink targets that cross those boundaries; current source inventory prunes descendant package-root subtrees and excludes child-owned symlink targets; the existing workspace module resolver therefore inherits the same rule without a second runtime authority. Focused evidence proves most-specific-root ownership, parent `self:`/entry rejection across member roots, canonical-root alias fail-closed behavior and runtime/static consistency. No filesystem package discovery, manifest search, source-set/glob authority, project-projection format or Protos language specification change is introduced. Implementation version becomes `0.2.415-SNAPSHOT`.

## 0.2.414-SNAPSHOT

- Implement `TOOL002-I8A — static Requirement-to-CatalogEntry binding`. Add private `ResourceBinding.protos` over the already-ratified D076/D077/D097 representations. Per-case canonical Requirements join to the selected validated catalog by logical key; resource-free input returns a frozen empty binding set; missing resource keys and `shared` units greater than total catalog capacity fail closed; exact-capacity shared requests and exclusive requests remain statically feasible; binding order follows requirement declaration order; scope/provider/profile remain inert CatalogEntry metadata. This slice deliberately has no scheduler occupancy, mutable reservation, placement-domain identity/compatibility, provider resolution/provisioning, `Main.protos` wiring or `Runner.protos` change, so it does not select fairness/topology/provider policy. I6E requirements discovery remains unwired until live D076 reservation can prevent a resourceful case from starting unreserved. Implementation version becomes `0.2.414-SNAPSHOT`.

## 0.2.413-SNAPSHOT
- Implement `TOOL002-I7B3B — public resource-catalog acquisition composition`. Add `ResourceCatalog.empty()` as the frozen inert D077/D099 no-source snapshot. Extend bundled Test Tool `Main.protos` to capture `process.args()` once, retain D069 `Options.jobs`, select D099 `Options.resourceCatalogPath`, skip acquisition when absent, and otherwise call the D101 private one-shot `catalogAcquirer`, decode its frozen raw Bytes through `Encoding.UTF8`, and reuse the existing I7A strict TOML-v1 `ResourceCatalog.parse`. Add focal composition evidence for no-source/no-acquirer, selected-path single acquisition plus decoded catalog fields, malformed-catalog fail-closed behavior, and static public-Main ownership guards. This completes the D099/D101 acquisition path only: the catalog is materialized/validated but is not yet used by Runner for D076 admission/reservation; provider implementation/loading remains deferred. Implementation version becomes `0.2.413-SNAPSHOT`.

## 0.2.412-SNAPSHOT
- Ratify `D102 — Nested workspace package source ownership and module-boundary contract` (GitHub #421) as Candidate A′ after explicit project-owner approval and exhaustive comparison across Bazel, Buck2, Go modules/workspaces, SwiftPM, Cargo, Gradle, Maven, sbt, Mill, Node/npm/Yarn/pnpm, BSP, Pants, MSBuild/.NET, Python/uv and CMake. Select canonical non-overlapping workspace package source domains: a strictly-descendant authorized package root is a hard boundary for each ancestor, the most-specific authorized root uniquely owns a physical source, and distinct PackageIds that materialize onto the same canonical physical package root fail closed. Require source inventory, exact source lookup, runtime module resolution and static tooling to share this boundary; package boundaries come only from canonical package/project authority, never nearest-manifest discovery, and physical containment cannot make a child package source reachable through parent `self:` lookup. Ratification is governance/package-system architecture only: no executable implementation, core specification, native-boundary or Maven implementation-version change. LM009-G3P P3B / #413 is now `BLOCKED_BY_D102_IMPLEMENTATION` until the shared package-source boundary is implemented.

- Implement `I038-D1 — Basic matching execution` (GitHub #391) over the already-ratified D071/D072/D073/D088/D092/D095 contracts. Add production AST execution for ordinary matcher/value patterns, explicit `@name` binders and `_` discard without creating runtime Match/Case/Pattern objects. Evaluate the subject exactly once; consider arms in source order; evaluate an attempted matcher expression exactly once; invoke only ordinary `pattern.match(subject)`; consume exactly canonical `false`, canonical `true`, or a non-empty standard Array under D072; shallow-snapshot capture arguments; and invoke the selected arm body as an ordinary generated Closure using D088 fixed/rest positional parameter binding. Invalid normal matcher outcomes and complete no-match signal fresh ordinary generic `Error`; a selected-arm arity failure remains an ordinary invocation Error and does not retry later arms. Standard Array/Map patterns, aliases, OR, guards, static coverage optimization and Bytecode DSL match execution remain later I038 slices. This intermediate publication uses `FOCAL_BOUNDED`; full integrated validation remains debt of top-level I038 closure. Implementation version becomes `0.2.412-SNAPSHOT`.
...
- Implement `PERF006 PLAT028 Environment.each C-prime callback orchestration`. Give the hidden standard `Environment.each` implementation stable native provenance and recognize structured execution only when the exact standard body is selected for an Environment snapshot whose exact construction prototype is the selected method home and that home was created by the hidden standard Environment prototype factory. Materialize the complete portable entry snapshot before callback #1, preserving the existing all-or-nothing Unicode representability cutover and Unicode-scalar sort order. Keep iteration state in C-prime: pass `(name, value)` left-to-right, ignore callback results, advance only after each callback truly completes, and return the exact Environment receiver. A suspending callback therefore resumes at the exact loop point without retaining a Java `for` frame or replaying the completed prefix. Retain the Java loop solely as the staged AST/native compatibility branch until B6B. Add C-prime whole-snapshot prevalidation, sorted suspension/resume, and strict anti-alias provenance focal evidence. No public API or Protos-visible semantics change. Implementation version becomes `0.2.409-SNAPSHOT`.

## 0.2.408-SNAPSHOT

- Implement `TOOL002-I7B2 — D101 one-shot catalog acquisition capability`. Add the private bootstrap-local `ProtosTestToolCatalogAcquisitionFacility` and install it only in the initial bundled Test Tool activation. Capture the invocation working directory at the `protos test` host boundary; expose only one native Closure `catalogAcquirer`, not a Filesystem; consume its authority on the first attempt; accept exactly one already-selected String PATH; resolve relative paths against the captured cwd and absolute paths directly; follow ordinary symlinks; require a readable regular final target; read the complete content once; and return caller-Prelude frozen raw Bytes without UTF-8/TOML/schema interpretation. Focal evidence covers relative/absolute paths, raw non-UTF8 bytes, symlinks, invalid/non-regular/missing targets, one-shot consumption after success or failure, argument validation and bootstrap-local non-Filesystem confinement. Public `Main.protos` composition remains I7B3; no D076 reservation/provisioning or provider implementation is included. This adds the private D101-ratified Test Tool native bootstrap boundary. Implementation version becomes `0.2.408-SNAPSHOT`.

## 0.2.407-SNAPSHOT
- Implement `PERF006 PLAT028 ProcessArguments.each C-prime callback orchestration`. Give the hidden standard `ProcessArguments.each` implementation stable native provenance and recognize structured execution only when the exact standard body is selected for a represented ProcessArguments receiver whose runtime construction prototype is the selected method home and that home was created by the hidden standard ProcessArguments prototype factory. Keep callback orchestration in C-prime state: preserve the immutable argument snapshot and original order, ignore callback results, advance only after each callback truly completes, and return the exact snapshot receiver. A suspending callback therefore resumes at the exact loop point without retaining a Java `for` frame or replaying the completed prefix. Retain the Java loop solely as the staged AST/native compatibility branch until B6B. Add exact-home/anti-alias provenance and suspension/resume focal evidence. No public API or Protos-visible semantics change. Implementation version becomes `0.2.407-SNAPSHOT`.

## 0.2.406-SNAPSHOT

- Implement `TOOL002-I7B1 — bundled-Protos resource-catalog option selection` under ratified D099/D101. Extend private Test Tool `Options.protos` with exact `--resource-catalog PATH` ownership through ordinary argument traversal: absence selects no source, one occurrence returns the exact following PATH token without path interpretation, and missing/repeated occurrences fail closed. Snapshot parser state before consuming a value so a selected PATH token is literal even when it begins with `-`. Preserve D069 `--jobs` semantics and prove both options compose without moving CLI policy into Java. Host path resolution/acquisition, the D101 one-shot immutable-Bytes capability, public Main composition, D076 admission/reservation and provider implementation remain deferred to I7B2/I7B3 and later slices. Implementation version becomes `0.2.406-SNAPSHOT`.

## 0.2.405-SNAPSHOT
- Ratify `D101 — Test Tool resource-catalog CLI ownership and host acquisition authority contract` (GitHub #420) as Candidate C′ after explicit project-owner approval and exhaustive comparison across CTest, kubectl/kubeconfig, Cargo, Terraform, Docker Compose, Bazel, Buck2, pytest, npm, Maven, Gradle, Slurm, Deno and WASI/cap-std. Keep D099 option spelling/cardinality/path-selection policy in bundled Protos through `process.args()`; Java must not separately reinterpret `--resource-catalog`. Select one Test-Tool-private one-shot catalog-acquisition capability that accepts only the chosen PATH, grants no broad/list/write/general Filesystem authority, resolves D099 absolute/cwd-relative path semantics, requires a readable regular final target, acquires the complete source once and returns an immutable Bytes snapshot. Keep UTF-8/TOML/I7A validation in bundled Protos, discard path/capability before scheduling, never delegate acquisition authority to child test Processes/providers, and require future managed/CAS/service sources to use distinct explicit adapters rather than reinterpreting v1 PATH. Governance/tooling only: no executable implementation, specification, native-boundary or Maven implementation-version change.

- Implement `PERF006 PLAT028 Bytes.each C-prime callback orchestration`. Give the canonical runtime `Bytes.each` implementation stable native provenance and route only its exact standard selection through C-prime-owned structured callback control. Snapshot the represented Bytes once at entry, preserve left-to-right octet order, ignore callback results, advance the cursor only after each callback truly completes, and return the exact Bytes receiver. A suspending callback therefore resumes at the exact C-prime loop point without retaining a Java `for` frame or replaying the completed prefix. Retain the existing Java loop only as the temporary AST/native compatibility path until B6B; selector/API, Bytes mutation/lifecycle semantics and Protos-visible semantics are unchanged. Add suspension/resume and mutation-during-iteration snapshot focal evidence. Implementation version becomes `0.2.405-SNAPSHOT`.

## 0.2.404-SNAPSHOT

- Implement `I038-C — Canonical matching IR` (GitHub #391) under ratified D071-D100. Add backend-neutral compiler-only `CanonicalMatch` / `CanonicalMatchPattern` carriers and recursively canonicalize the published I038-B surface form: subject, ordinary matcher expressions, Map key expressions, guards and arm bodies become ordinary canonical expressions; D095 binder/capture/remainder structure is preserved; surface-only grouping disappears from pattern IR. This slice creates no Protos runtime Match/Case/Pattern object, registry or alternate matcher authority and deliberately does not add Truffle/Bytecode match execution. Existing backend lowerers remain fail-closed for the newly canonicalized form until the next I038 execution slice. This is an intermediate `FOCAL_BOUNDED` I038 publication; full integrated validation remains debt of top-level I038 closure. Implementation version becomes `0.2.404-SNAPSHOT`.

## 0.2.403-SNAPSHOT

- Implement `I038-B — Matching surface AST and parser` (GitHub #391) under ratified D093/D095/D096/D100. Add a dedicated non-runtime `SurfaceMatch` / `SurfaceMatchPattern` source model and parse the contextual postfix `match { case ... }` envelope, explicit `@` binders/aliases, `_`, standard Array/Map/exact/remainder forms, ordered single-`|` OR, guards, and opaque `captures(...)` consumer interfaces without adding reserved words or lexer tokens. Enforce the ratified syntax-stable source checks: at least one arm, unique/linear binder names, equal ordered binder-name interfaces for fixed OR alternatives, no arm after an unguarded syntactically irrefutable arm, and no OR alternative after an already reachable syntactically irrefutable alternative. D100 is implemented delimiter-first at the guard root: the first structurally top-level `=>` closes the guard, while grouped/nested Closures remain ordinary. D095 `exact %Ellipsis` plus a remainder is rejected. The D079 document-symbol walker explicitly traverses match subjects, matcher/key expression positions, guards and arm bodies so the sealed surface tree remains total without inventing Match symbols. Arbitrary matcher values remain ordinary literal/name/intrinsic plus member/call/index postfix source; no runtime matching lowering, canonical AST, coverage engine, matcher metadata, Pattern hierarchy or specification change is included. This is an intermediate `FOCAL_BOUNDED` I038 publication; full integrated validation remains debt of top-level I038 closure. Implementation version becomes `0.2.403-SNAPSHOT`.

## 0.2.402-SNAPSHOT

- Implement `TOOL002-I7A — resource catalog v1 schema parser`. Add the private bundled-Test-Tool `ResourceCatalog` parser over already-acquired TOML 1.0 text, reusing the D087 shared TOML authority and enforcing the ratified D077/D097/D098 catalog contract: version 1 only, strict fields, canonical logical resource key, positive ordinary Integer capacity, scope exactly `placement` or `run`, mandatory canonical inert logical provider, optional canonical provider-scoped profile, declaration-order preservation and duplicate-key rejection. Add focused parser evidence for valid/empty catalogs and malformed version/shape/identity/capacity/scope/duplicate cases. Reconcile the TOOL002-I work row after published I6E. D099 file/CLI acquisition, D076 reservation/provisioning, provider implementation/loading, Main wiring and public `std:toml` remain excluded. Implementation version becomes `0.2.402-SNAPSHOT`.

## 0.2.401-SNAPSHOT
- Implement `PERF006-B6A6A3 — PLAT027 Actor/Group host-lifecycle integration`. Keep ordinary message lookup and handler selection authoritative, but when ActorRequest, GroupRequest, SendOperation or GroupSendOperation selects a Task-owned C-prime guest handler, install the already-published PLAT027 one-shot terminal lifecycle boundary immediately before C-prime execution. True-terminal completion now performs the existing reply transfer/delivery completion or Actor failure/cancellation lifecycle after C-prime suspension/resume, cancellation unwind and child drain instead of relying on a discarded Java caller frame. The native/AST compatibility branch remains the existing post-return Task-state switch until B6B; remote transport paths are unchanged. Add retained suspension evidence for all four local consumers and preserve exact request result transfer, send-result ignoring, accepted-delivery failure/cancellation consequences and Group operation completion. No Protos-visible semantics or public API change. Implementation version becomes `0.2.401-SNAPSHOT`.

## 0.2.400-SNAPSHOT

- Implement `LM009-G3P-P3A — canonical protos.project producer primitives` (GitHub #413; parent #373) under ratified D082 A′ / D085 F′ / D089 A″. Add Package-Tool-owned `ProjectDocument`, `ProjectMetadata` and `ProjectFile` modules that canonically render generation-1 `protos.project`, compute the exact `protos-project-metadata-v1` SHA-256 framing over root/member manifests plus `protos.lock`, preserve the existing `protos-resolution-input-v1` semantic witness, reject stale or mismatched lock/project identity, and expose an atomic publication primitive through the existing metadata publication substrate. Ordinary `.protos` source bytes remain outside project-metadata freshness. This slice adds no public Package Tool command, language-server reader/provider, project discovery, automatic guest refresh, workspace-symbol index, definition semantics or specification/native-boundary change. This intermediate P3A publication uses the declared `FOCAL_BOUNDED` profile; broader integrated validation remains debt of LM009-G3P-P3 / GitHub #413 closure. The Maven implementation version remains `0.2.400-SNAPSHOT`.
...
- Ratify `D099 — Test Tool resource catalog source and CLI acquisition contract` (GitHub #414) as refined Candidate A′ after explicit project-owner approval and exhaustive comparison across CTest, Kubernetes/kubectl, Terraform, Docker Compose, Nomad, Slurm, Bazel, pytest, Cargo, Maven and Gradle. Select optional `protos test --resource-catalog PATH` with exactly zero or one occurrence: absence means the D077 empty catalog, one occurrence acquires exactly one explicit local readable regular file, and repetition fails closed with no first/last-wins or merge semantics. Interpret relative paths against the invocation working directory; introduce no default filename, cwd/parent/home/system/repository search, environment-variable fallback, corpus/manifest binding, directory expansion, stdin, URL/URI or generated source in v1. Acquire once and feed the strict D077/D097/D098 catalog boundary; the host path is not catalog identity and must not enter TestPlan/CaseSpec, provider/profile identity, guest state or future remote-worker path requirements. Resourceful cases remain subject to D076 admission even when no catalog is supplied. Keep future managed/CAS/service acquisition as a separate explicit extension rather than reinterpreting v1 `PATH`. Governance/tooling only: no specification, executable implementation, native-boundary or Maven implementation-version change.

- Implement `PERF006-B6A6A2 — PLAT027 Task terminal lifecycle primitive`. Add one optional backend-private, one-shot terminal lifecycle boundary to `ProtosTask`; ordinary Tasks retain only a null reference and allocate no lifecycle carrier. Route completed, failed and cancelled true-terminal publication through one bounded helper that preserves existing domain/associated-Future ordering, consumes the finalizer exactly once only after child-drain/cancellation-unwind terminalization, and lets escaping Java finalizer failures remain runtime invariant failures rather than guest outcomes. Add focal evidence for completion, failure, cancellation, structured-child drain, one-shot cardinality and invariant-failure behavior. Actor/Group request/send integration remains deferred to B6A6A3. Implementation version becomes `0.2.398-SNAPSHOT`.

## 0.2.397-SNAPSHOT

- Implement `I038-A — Root matcher foundation` (GitHub #391) under ratified D073/D081. Publish inherited `Object.match(subject)` as a source-backed standard Object Closure whose body performs exactly one ordinary pattern-side `this == subject` send and returns that exact result unchanged. Retain a Protos-level conformance case proving dynamic equality selection, exact result identity and exactly-once invocation, plus bootstrap/context-local-projection/native-boundary regressions. No matcher registry, native matcher authority, syntax implementation, composite/Array/Map/OR/guard execution, coverage analysis or specification change is included. This is an intermediate `FOCAL_BOUNDED` I038 publication; unrestricted integrated Maven validation remains debt of top-level I038 closure. Implementation version becomes `0.2.397-SNAPSHOT`.

## 0.2.396-SNAPSHOT

- Repair `PERF006-B6A6A1R - legacy AST fallback for Bytecode-plan Closures`. B6A6A1 intentionally retained the historical AST/replay path as the temporary B6B oracle/fallback, but a Bytecode-backed Closure reaching that path was being reprojected as Bytecode and then rejected by the staging sentinel. Add an explicit Context-local AST re-lowering seam from the exact canonical Closure definition/source, route synchronous legacy invocation through that AST cache whenever a Bytecode plan reaches an entered Context, and keep explicit Task-owned entry on the C-prime Bytecode projection cache unchanged. This restores pre-cutover host/runtime consumers without treating `ContinuationResult` as synchronous completion and without implementing B6A6A2/B6A6A3. Add focal evidence that the semantic Closure remains Bytecode-backed while the temporary legacy projection is AST-backed. Entered-Context AST projections are language-bound to the owning ProtosLanguageContext, preserving nested Closure ownership and the existing A+ one-template/one-projection cardinality. Only the no-entered-Context temporary fallback uses language-unbound `sourceOnly` roots (or legacy roots when no Source exists). Entered-Context invocation uses the exact ProtosLanguageContext projection cache only for explicitly context-local, language-unbound, or Bytecode-backed templates; when no Polyglot Context is entered inside a cooperative Task, the fallback plan is cached by that Task's `ProtosEvaluatorContinuation`, preserving exact `ProtosExpressionNode` identity across replay segments while preventing AST sharing across Tasks. Direct non-Task fallback remains fresh. `ProtosSourceCompiler.compile(String)` now preserves its exact characters in a language-unbound `sourceOnly` Truffle Source, so direct/harness Closures retain the Source required if they later enter C-prime composition. C-prime Bytecode projection remains language-bound. Legacy/AST `await()` boundaries reached while the same Task is executing a C-prime segment now select the existing continuation-cancellation unwind entry, so Bytecode structured cleanup sees the exact cancellation transfer before Task terminalization; ordinary AST-only cancellation keeps the legacy observation path. During retained Actor-corpus validation this repair also exposed a pre-existing `Future.then` register/suspend race: a source can become terminal after observer registration but before `ProtosTask.suspend()`, whose intentional ready-before-suspend result is `false` with the Task still `RUNNING`. Honor that result instead of unconditionally returning from the producer continuation, so the already-ready source is projected immediately and downstream `then` Futures are not stranded. No Task/Future semantics change; this restores the existing lost-wakeup contract. The same recovery train also implements the first PLAT028 production callback cutover: canonical Boolean `ifTrue`, `ifFalse`, `ifTrueIfFalse`, `and`, and `or` retain their short-circuit/selected-only semantics while callback orchestration and post-callback validation move into C-prime composed control. `not` remains a native leaf. This prevents an inner Task's replay-era `ProtosEvaluatorSuspension` from escaping through a Boolean Java frame into an outer Task while preserving ordinary selector overrides. The same PLAT028 cutover now migrates canonical `Array.each`: callback validation and the entry snapshot are preserved, while the snapshot cursor and repeated guest callback invocations are owned by C-prime resumable control rather than a Java `for` frame. Legacy/non-C-prime invocation retains the compatibility implementation during the B6 cutover. Structured C-prime `ensure` now snapshots whether cancellation was already unwinding before cleanup starts: only that pre-existing cancellation may be superseded by a later cleanup transfer, while cancellation initiated by cleanup correctly supersedes an earlier selected Error/return/normal outcome. Implementation version becomes `0.2.396-SNAPSHOT`.

## 0.2.395-SNAPSHOT

- Ratify `D098 — Test Tool resource catalog provider/profile identity contract` (GitHub #393) as Candidate A′ after explicit project-owner approval and exhaustive comparison across Kubernetes DRA/CSI, Nomad, Buck2, Bazel, CTest, Slurm, Terraform, CNI, Docker and Jenkins. Require one explicit canonical inert logical `provider` identity per catalog resource plus an optional provider-scoped canonical inert `profile`; never infer provider from `resource.key`; reuse the canonical `segment[/segment...]` lexical grammar while keeping resource/provider/profile namespaces distinct; keep Java/Protos/native implementation names, executables, URLs/URIs, package coordinates, arbitrary provider-specific inline maps, secrets, credentials and live authority out of catalog v1. The scheduler transports provider/profile after generic placement/reservation but does not interpret provider-internal semantics; unknown provider/profile fails closed as infrastructure/configuration evidence. Preserve a cheap future registry path from logical provider identity to source/package/version/ABI/configured implementation without changing Requirement, key, capacity or D097 scope semantics. Governance/tooling only: no specification, executable implementation, native-boundary or Maven implementation-version change.

- Ratify `D097 — Test Tool resource catalog scope/locality vocabulary contract` (GitHub #392) as Candidate B′ after explicit project-owner approval and exhaustive comparison across CTest, JUnit, cargo-nextest, pytest-xdist, GitHub Actions, GitLab CI, Jenkins, Buildkite, Azure Pipelines, Bazel/Remote Execution, Buck2, Kubernetes DRA, Nomad, Slurm and HTCondor. For `resource-catalog-version = 1`, restrict `scope` to `placement` or `run` and define it only as the scheduler-visible capacity accounting/reservation domain: `placement` ties a pool to an execution-placement domain without selecting worker/host/node topology, while `run` is one logical pool shared across compatible placements in one Test Tool invocation. Keep attempt-private capability lifetime, provider lifetime/ownership, physical externality/topology and cross-run/global coordination orthogonal and deferred; unknown v1 scopes fail closed. Governance/tooling only: no specification, executable implementation, native-boundary or Maven implementation-version change.

- Ratify `PLAT028 — C-prime-owned suspendible callback orchestration and native-leaf boundary` (GitHub #386) as Candidate C after explicit project-owner approval and an exhaustive audit of all 16 principal Truffle implementations, relevant experimental/historical implementations, Apple Pkl, and outside-Truffle async runtimes. Require all post-callback control that may survive an ordinary guest suspension to remain in C-prime-resumable state; keep Java/native as guest-non-reentrant leaf mechanics while preserving PLAT019 native suspension and PLAT027 terminal-only finalization. Reject a generic Task-owned host continuation stack, operation-specific host continuation architecture as the default, artificial child Tasks, blocking carriers, AST/replay islands, whole-JVM-stack capture and a new non-suspendible-callback restriction. Release bounded PERF006-B callback-family migration/restoration work. Governance only: no specification, executable implementation, public API, native boundary or Maven implementation-version change.

- Ratify `D094 — Test Tool resource-requirements sidecar filename and discovery contract` (GitHub #388) as refined Candidate A′ after explicit project-owner approval and an expanded comparison across CTest, JUnit Jupiter, cargo-nextest, Bazel, Buck2, GitHub Actions, GitLab CI, CMake Presets, Cargo, Go modules, TypeScript, Gradle, pytest, Jest, Vitest, ESLint and Maven Surefire. Select the exact optional corpus-root sibling `resource-requirements.toml`, discovered only through the already-authorized corpus Filesystem using exact direct-child observation; true absence means zero requirements, while a present non-regular entry or any open/read/TOML/schema/D091 join failure fails closed. Reject public requirements-sidecar overrides, parent/cwd/home/env search, merge, aliases and link following; keep catalog source/CLI, scope vocabulary, provider APIs and a general metadata namespace deferred. Release TOOL002-I6E for bounded implementation. Governance/tooling only: no specification, executable implementation, native-boundary or Maven implementation-version change.

- Implement `PERF006-B6A6A1 — Task-owned Closure C-prime dispatch and Context-local Bytecode projection`. Keep the historical AST execution-plan projection as the temporary B6B differential oracle while adding a separate Context-owned Bytecode projection cache for source-backed Closures. Task-owned direct and polymorphic Closure entry now preserve ordinary D013/PLAT017 selection, project exact source+definition into the entered Process Context, and drive the resulting C-prime chain through the existing PLAT014/PLAT019 Task suspension machinery. Migrate standard Closure.future, Future.then, exact-process inspection and isolated-P root callbacks onto that boundary, and permit nested C-prime composition to project AST templates instead of rejecting them. Focal evidence covers genuine suspension/resume with no completed-prefix replay plus future/then dispatch. PLAT027 terminal lifecycle integration, PLAT026 production RootTag implementation, public parse cutover, PLAT028 native-wrapper guest callbacks and replay retirement remain out of scope. Implementation version becomes `0.2.395-SNAPSHOT`.

## 0.2.394-SNAPSHOT

- Implement `TOOL002-I6D — strict D091 requirement-to-CaseSpec join` under ratified D077/D091. Extend the private bundled-Test-Tool `ResourceRequirements` module with a complete-TestPlan join that indexes canonical `Manifest.caseId` identities, requires every I6C declaration to resolve to exactly one CaseSpec, fails closed for missing or referenced-ambiguous identities, groups declarations per case in declaration order, and rebuilds the complete plan through the existing I5 `caseSpecWithRequirements` path while preserving input case order and cardinality. Validation is therefore complete-plan referential validation before any later filter/shard selection; the requirement document creates no cases. This slice still selects no physical sidecar filename or filesystem discovery and adds no catalog/scope/provider/reservation/CLI/public `std:toml` behavior. Implementation version becomes `0.2.394-SNAPSHOT`.

## 0.2.393-SNAPSHOT

- Ratify `D091 — Test Tool orphan resource-requirement / CaseSpec join policy` (GitHub #380) as Candidate A′ after explicit project-owner approval and exhaustive comparison across CMake/CTest, JUnit Jupiter, Bazel, TypeScript, Terraform, Docker Compose, cargo-nextest, pytest and Kubernetes. Treat `requirement.case` as an exact intrinsic reference into the complete associated TestPlan, require every declaration to resolve to exactly one existing CaseSpec, and fail closed before execution when metadata is orphaned. Keep invocation filtering/sharding strictly after corpus referential validation; selected subsets never redefine orphanhood, requirements do not create phantom cases, and unresolved declarations do not flow to schedulers or workers. Release TOOL002-I6D for bounded join implementation while physical sidecar filename/discovery, catalog CLI/source, scope vocabulary, provider/reservation policy and public `std:toml` remain deferred. Governance/tooling only: no specification, executable implementation, native boundary or Maven implementation-version change.

- Ratify `PLAT027 — Task-owned terminal lifecycle composition around C-prime guest execution` (GitHub #378) as Candidate A′ after explicit project-owner approval and exhaustive review of all 16 principal + 21 experimental/historical Truffle catalogue entries including Apple Pkl, plus Rust/Tokio, Swift concurrency, BEAM, Java CompletionStage and Go. Select one backend-private Task-owned one-shot terminal lifecycle boundary that runs only after the same Task truly terminalizes, including C-prime suspension/resume, cancellation unwind and child drain; keep `ContinuationResult` as the sole guest continuation authority; retain operation-specific lifecycle state outside Task; forbid guest re-entry, suspension, generic host→guest→host state machines, artificial child Tasks, blocking carriers and AST/replay islands. Release PERF006-B6A6A for bounded normal Closure-dispatch / Context-local projection implementation after publication. Governance only: no specification, executable implementation, public API, native boundary or Maven implementation-version change.

- Ratify `D089 — ProjectBinding projection publication and acquisition mechanism` (GitHub #376) as Candidate A″ after explicit project-owner approval and exhaustive comparison across rust-analyzer/rust-project.json, CMake File API, Meson introspection, clangd compilation databases/index layering, SourceKit-LSP/BSP, Metals/Bloop/BSP, HLS/hie-bios, TypeScript ExternalProject, Bazel-oriented generated descriptions and gopls. Select Package-Tool-generated portable repository state `protos.project` with Package-Tool-owned `protos-resolution-input-v1` semantic SHA-256 plus independently recomputable `protos-project-metadata-v1` SHA-256 over exact root/member manifest and `protos.lock` bytes; exact candidate-root acquisition; fail-closed missing/stale state; no absolute persisted root, source inventory, project discovery, semantic manifest/lock reader or automatic Package Tool/guest fallback. Release LM009-G3P P3 for bounded producer/reader/provider implementation while LM009-G3 remains blocked until P3/P4 close. Governance/tooling only: no specification, executable implementation, native boundary or Maven implementation-version change.

- Implement `TOOL002-I6C — D077 resource-requirements TOML 1.0 schema parser` under ratified D077 and D087. Add private bundled-Test-Tool `ResourceRequirements` parsing over supplied text through the shared `Toml10/TomlDocument` authority, validating schema version 1, strict root/record fields and node types, canonical case paths, canonical resource keys, shared/exclusive units invariants, declaration order and duplicate `(case,key)` rejection, and materializing frozen inert `(case, Requirement)` declarations through the existing I2-I5 Manifest constructors/accessors. The slice intentionally performs no filesystem/sidecar discovery, selects no physical sidecar filename, does not join declarations to TestPlan CaseSpecs or choose unknown-case policy, and adds no catalog/scope/provider/reservation/CLI/public `std:toml` behavior. Implementation version becomes `0.2.393-SNAPSHOT`.

## 0.2.392-SNAPSHOT

- Ratify `PLAT026 — Bytecode production root-tag compatibility boundary` (GitHub #375) as Candidate C′+ after explicit project-owner approval and expanded Truffle tooling review covering GraalPy Bytecode DSL, SimpleLanguage, TruffleRuby, GraalJS, Espresso, Sulong/LLVM, GraalWasm, FastR, TruffleSqueak, Enso and Apple Pkl, plus the GraalVM DAP `SourceElement.ROOT` and Bytecode DSL root-prolog contracts. Add `RootTag` only for truthful semantic top-level/module/Closure activation roots through automatic Bytecode DSL root tagging; keep Object-construction/helper roots untagged, keep `RootBodyTag` deferred, and keep physical `RootNode`/`CallTarget` identity non-authoritative. Release the tooling/root-tag portion of PERF006-B6A6 for bounded implementation. Governance only: no Protos specification, executable implementation, public API, native boundary or Maven implementation-version change.

- Implement `TOOL002-I6B — shared TOML 1.0 authority migration` under ratified D087 Candidate B and the I6A private resolver boundary. Move the single bundled-Protos TOML syntax/document implementation from Package Tool ownership to the private version-pinned `protos/tools/shared/Toml10/` bootstrap root, retain tiny Package-owned `self:TomlSyntax` / `self:TomlDocument` compatibility adapters whose private shared imports are resolved during authorized module initialization and then captured lexically without duplicating parser mechanics, and explicitly configure Package Tool resolver consumers with the sibling shared root. The retained exact-source Package/TOML corpus keeps its historical `self:` boundary and never receives ambient `tool-shared:` authority. This slice adds no D077 requirement-document schema/parser, physical sidecar filename, catalog, scope vocabulary, provider/reservation behavior, public `std:toml`, general internal tool package graph, specification change or native-boundary change. Implementation version becomes `0.2.392-SNAPSHOT`.

## 0.2.391-SNAPSHOT

- Implement `LM009-G3P-P2 — inert immutable ProjectBinding/projection carriers` (GitHub #373) under ratified D082 A′ / D085 F′. Add an editor-neutral immutable `ProtosProjectBindingProjection` carrying one canonical project-root witness, exact workspace PackageId/location membership, generation and opaque package-authority freshness witness, plus an immutable `ProtosProjectBinding` snapshot that binds the projection to exact package roots and already-validated current source identities. Defensively freeze collections and fail closed on structural inconsistency, duplicate package/source identity, incomplete projection coverage, root-witness mismatch and source escape. The carriers do not read manifests/locks, compute freshness, discover projects, enumerate files, execute guest code, select a persistence/serialization/filename mechanism, acquire candidate roots or implement workspace-symbol indexing. LM009-G3 remains blocked on P3/P4 provider/acquisition integration and prerequisite closure. Implementation version becomes `0.2.391-SNAPSHOT`.

## 0.2.390-SNAPSHOT

- Implement `LM009-G3P-P1 — canonical bounded workspace-package source inventory` (GitHub #373) under ratified D082 A′ / D085 F′. Add an immutable deterministic inventory over already-authorized workspace package roots only: derive portable logical-module candidates from current `.protos` sources, reuse the existing runtime-name and exact `ProtosWorkspacePackageSourceLookup` round trip as authority, exclude sources escaping a package root and fail closed on exact-module ambiguity. Add no project discovery, manifest/lock interpretation, dependency/stdlib inventory, open-document overlay, workspace-symbol indexing, definition behavior, guest execution or projection persistence/acquisition mechanism. LM009-G3 remains blocked on the remaining ProjectBinding implementation prerequisite. Implementation version becomes `0.2.390-SNAPSHOT`.

## 0.2.389-SNAPSHOT

- Implement `TOOL002-I6A — private shared bundled-tool resolver boundary` under ratified D087 Candidate B. Extend the existing bundled-tool resolver with one explicitly configured private shared-bootstrap source root and an implementation-private `tool-shared:` import spelling, keep `self:` exactly tool-local, assign shared modules a stable toolchain-private `bundled-tool-shared:` ModuleKey independent of the consuming bundled tool, and fail closed for missing shared authority, imports without an owning bundled/shared importer, ambient importer identities, malformed/case-mismatched paths, or shared modules attempting to use `self:` as tool-local authority. This slice changes no TOML parser source, D077 persisted schema, sidecar filename, catalog CLI/source, scope vocabulary, provider/reservation semantics, public `std:toml`, specification or native boundary. Implementation version becomes `0.2.389-SNAPSHOT`.

## 0.2.388-SNAPSHOT

- Ratify `D085 — Static canonical Project Binding provider and language-server acquisition contract` (GitHub #370) as Candidate F′ after explicit project-owner approval and exhaustive comparison across rust-analyzer, SourceKit-LSP/BSP, Metals/BSP, clangd, HLS/hie-bios, gopls, TypeScript/tsserver, Pyright, Ruby LSP, Eclipse JDT LS, OCaml-LSP/Dune/Merlin, Dart Analysis Server and ElixirLS. Select a Package-owned minimal canonical `ProjectBinding` projection, bounded current source inventory only inside canonically authorized workspace package roots, exact logical-module/source round trips, and validated candidate-root acquisition through a replaceable editor-neutral provider. Reject project discovery by workspace-folder/path heuristics, a second LSP manifest/lock authority, a complete high-churn source-list sidecar, and mandatory guest/daemon execution as baseline. D082 remains authoritative for per-project indexing and live overlays. LM009-G3 remains blocked only on bounded ProjectBinding prerequisite implementation; governance/tooling only, with no specification, production implementation, native-boundary or Maven implementation-version change.

- Ratify `D087 — Shared bundled-tool TOML bootstrap authority and import boundary` (GitHub #372) as refined Candidate B after explicit project-owner approval and exhaustive comparison across LLVM/Clang, Go internal packages, Rust/Cargo, Swift/SwiftSyntax, TypeScript, Roslyn/.NET, Java/JDK tooling and Python `tomllib`. Select one private toolchain-owned shared TOML syntax/document layer implemented in Protos source, reachable only through bundled-tool-private resolution while preserving tool-local `self:` and separate Package Tool/Test Tool schema ownership. Require persisted schema generations to pin their TOML dialect explicitly (current D077/package generations remain TOML 1.0), keep project/package/runtime resolvers outside the private namespace, and leave public `std:toml`, a general internal tool package graph and all D077-deferred sidecar/catalog/provider/scope contracts unselected. Release the persistent-TOML continuation of TOOL002-I. Governance/tooling only: no Protos specification, production implementation, native boundary or Maven implementation-version change.

- Implement `PERF006-B6A5 — module initialization C-prime bridge` under ratified PLAT025 Candidate A′. After ordinary lookup selects the exact canonical standard `import.call` bridge, specialize only that selection into a backend-private module-lifecycle carrier around one nested Bytecode C-prime module root. Preserve Actor-local cache-before-execute and immediate READY/INITIALIZING cache hits; attach the child module activation to the importing Task/dynamic-control flow; keep the exact record INITIALIZING across yield; mark READY only after true child completion while returning the module instance rather than the body value; and remove only the exact active record when Error, cancellation or another terminal transfer escapes. Keep `ContinuationResult` as the sole interpreter continuation representation, retain ordinary behavior for shadowed/aliased non-canonical import calls, and add focal evidence for recursive cycles, real `Future.value()` suspension without replay, READY hits, terminal failure/cancellation/retry and post-lookup exact-standard selection. No production `ProtosLanguage.parse()` cutover, generic native-to-guest ABI, child Task for module initialization, blocking carrier, async-module graph, replay retirement, specification or public API change is included. Implementation version becomes `0.2.388-SNAPSHOT`.

## 0.2.387-SNAPSHOT

- Implement `TOOL002-I5 — CaseSpec requirement attachment and duplicate rejection` under the ratified D076/D077 resource model. Add a private in-memory CaseSpec attachment path that rebuilds supplied Requirement shapes through the canonical I3/I4 validator, preserves declaration order by ordinary `Array(...validated, canonical)` reconstruction into a fresh frozen requirements Array, and fails closed when one case declares the same canonical resource key more than once, including shared/shared and shared/exclusive duplicates. The same logical key remains valid across different CaseSpecs. This slice adds no persistent requirement TOML, physical sidecar filename, catalog, reservation/capacity, provider/capability API, CLI surface, public scope vocabulary, specification change or native-boundary change. Implementation version becomes `0.2.387-SNAPSHOT`.

## 0.2.386-SNAPSHOT

- Ratify `D082 — Workspace-symbol project, source-set, and index authority` (GitHub #367) as Candidate A′ after explicit project-owner approval and an expanded comparison across rust-analyzer, clangd, gopls, HLS/hie-bios, SourceKit-LSP, TypeScript/tsserver, Pyright, Ruby LSP, Eclipse JDT LS, OCaml-LSP/Dune/Merlin, Dart Analysis Server and ElixirLS. Select canonical editor-neutral Project Bindings, per-project incremental symbol-index domains, exact open-document overlays, session-level workspace-symbol aggregation, local-workspace-only baseline scope, lazy/pay-for-use construction and session-scoped index lifetime while preserving persistence/remote/dependency indexes and process-per-root hosting as future optional layers. Keep LM009-G3 blocked on the missing canonical `ProjectBinding + source inventory` prerequisite; do not substitute filesystem scanning, open-document-only workspace results or Package Tool guest execution. Governance/tooling only: no Protos specification, production implementation, native boundary or Maven implementation-version change.

- Ratify `PLAT025 — Bytecode module-initialization composition boundary` (GitHub #368) after explicit project-owner approval and exhaustive review of all 16 principal + 21 experimental/historical Truffle catalogue entries, including Apple Pkl, GraalJS, Espresso, TruffleRuby, GraalPy, FastR, GraalWasm, SOMns, TruffleSqueak, Yona, PorcE, Mozart-Graal and StreamBlocks, plus outside-runtime lifecycle evidence and the required future/scalability/Protos-philosophy stress review. Select Candidate A′: after ordinary lookup selects the exact canonical standard `import` implementation, Bytecode may specialize it into a backend-private module-lifecycle carrier around one nested C-prime module child root. Keep `ContinuationResult` as the sole interpreter continuation authority; keep READY/INITIALIZING cache hits immediate; on cache miss preserve cache-before-execute, mark READY only after true child completion, and remove the exact active record on terminal failure/cancellation. Reject a module-specific second continuation family, generic native-to-guest-call ABI, child Task, carrier blocking, ECMAScript-style async module graph and AST/replay island. Release PERF006-B6A5 for bounded implementation. Governance only: no specification, executable implementation, Maven implementation-version or module semantic change is included.
