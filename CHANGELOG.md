...
- Ratify `WEB001-F — Production hosting architecture` (GitHub #294) after explicit project-owner approval: supersede only the GitHub Pages hosting component of WEB001-B with a single self-hosted production path consisting of a generic immutable static website image in the public `guillermomolina/protos-website` repository and environment-specific Compose/Traefik configuration in private `guillermomolina/docker-home`. Keep exact-SHA Protos source authority, Astro + Starlight, static output, provenance and future playground isolation unchanged; retire the dormant Pages deployment path in the later implementation slice. No website implementation, DNS/production activation, Protos specification/runtime, public API, license term, or implementation-version change.
