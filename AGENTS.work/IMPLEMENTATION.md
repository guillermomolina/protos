...
During language version "0.1", the design may evolve, but implementation changes must still follow the current speci
...
If existing implementation behavior and the current specification disagree, the specification wins unless the user e
...
Before adding a dependency, check whether the JDK, GraalVM/Truffle platform, or an existing project dependency alrea

Do not upgrade Java, GraalVM, Truffle, build tooling, parser libraries, or other foundational dependencies as part o
...
Do not normally let a hand-written source file exceed 800 lines; split it into cohesive components before adding sub
...
Before implementing or changing a feature, inspect nearby code and comparable features to identify established proje

Prefer extending existing lexer, parser, AST, runtime, protocol, and Truffle abstractions over introducing parallel 
...
repository state needed to establish semantic and implementation preconditions,
make the bounded code/test changes, inspect that delta, and hand the human the
required focal/affected validation commands. A starting HEAD or expected version
is evidence about those preconditions, not a reservation of repository-global
publication state.

During that substantive implementation phase, do **not** modify `pom.xml` or the
root `CHANGELOG.md` merely because the eventual commit will require implementation
metadata. Do not reserve or predict the next Maven version. Finish the substantive
delta first and wait for the human to report the requested focal/affected
validation result. If validation fails, continue repairing and validating the
substantive delta without materializing moving publication metadata.

Only after the human reports the required substantive validation green does the
work enter **finalization**. At that point, re-inspect the current changed-path
set and current HEAD, and re-read the current `pom.xml`, root `CHANGELOG.md`, and
relevant `origin/main` state. Derive the then-current next implementation version
and add the matching version/changelog metadata. If another agent or publication
consumed an earlier expected version, recompute from current state; do not discard
or restart otherwise valid substantive work unless the concurrent change actually
overlaps or invalidates one of its semantic, implementation, or validation
preconditions.

Finalization metadata is repository-global shared state and MUST be based on the
state that exists when finalization begins, not on the state observed when
implementation began. The finalizer MUST preserve unrelated concurrent work and
MUST NOT fold another agent's substantive changes into this task merely because
they are present in the checkout.

Late metadata materialization does not weaken commit atomicity. The required
version bump and matching changelog entry MUST still be present in the same final
commit as the implementation change. After metadata is materialized, perform the
remaining final-candidate checks applicable to the resulting exact bytes.
Metadata-only finalization does not by itself require repeating already-green
behavioral tests unless the metadata edit, concurrent repository movement, or
validation policy changes executable/dependency state or otherwise invalidates
that evidence.
