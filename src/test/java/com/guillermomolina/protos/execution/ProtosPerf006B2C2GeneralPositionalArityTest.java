...
final class ProtosPerf006B2C2GeneralPositionalArityTest {
    ...
    @Test
    void nestedCallArgumentsAndDefaultRestBindingRemainFailClosed()
            throws Exception {
        try (Context context = Context.newBuilder(ProtosLanguage.ID).build()) {
            context.initialize(ProtosLanguage.ID);
            context.enter();
            try {
                ...
                String restCharacters = "(a, ...rest) => { a }";
                ...
                Source restSource =
                        Source.newBuilder(
                                        ProtosLanguage.ID,
                                        restCharacters,
                                        "perf006-b2c2-rest.protos")
                                .build();
                UnsupportedOperationException restFailure =
                        assertThrows(
                                UnsupportedOperationException.class,
                                () ->
                                        new ProtosBytecodeClosureExecutionPlan(
                ...
                assertTrue(
                        restFailure.getMessage()
                                .contains("rest parameter binding is deferred"));
            } finally {
                context.leave();
            }
        }
        ...
        System.out.println(
                "PERF006_B2C2_DEFAULT_REST_BINDING_DEFERRED=PASS");
    }
