...
public final class ProtosMapValue extends ProtosObjectValue {
 ...
 public List<Entry> keyedSnapshot(){return List.copyOf(entries);} public List<Map.Entry<Object,Object>> associationSnapshot(){ArrayList<Map.Entry<Object,Object>> snapshot=new ArrayList<>(entries.size());for(int index=0;index<entries.size();index++){Entry entry=entries.get(index);snapshot.add(Map.entry(entry.key(),entry.value()));}return List.copyOf(snapshot);} public boolean comparisonActive(){return comparisonDepth!=0;}
