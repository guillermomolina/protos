...
public final class ProtosFutureValue extends ProtosObjectValue {
    ...
    /** Runtime-only non-adopting completion used by lifecycle observations. */
    boolean resolveRuntimeValue(Object result) {
        Objects.requireNonNull(result, "result");
        if (result instanceof ProtosFutureValue) {
            throw new IllegalArgumentException("runtime lifecycle result cannot adopt a Future");
        }
        return transition(State.RESOLVED, result, null);
    }
