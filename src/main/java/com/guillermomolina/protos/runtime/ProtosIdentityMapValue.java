...
public final class ProtosIdentityMapValue extends ProtosObjectValue {
 ...
 private final List<Entry> entries=new ArrayList<>(); public ProtosIdentityMapValue(Object parent){super(parent);} public int keyedSize(){return entries.size();} public List<Entry> keyedSnapshot(){return List.copyOf(entries);} public List<Map.Entry<Object,Object>> associationSnapshot(){return entries.stream().map(e->Map.entry(e.key(),e.value())).toList();} public void append(Object k,BigInteger h,Object v){entries.add(new Entry(k,h,v));} public void replaceValue(Entry e,Object v){e.value(v);} public Object remove(Entry e){if(!entries.remove(e))throw new IllegalStateException("foreign IdentityMap entry");return e.value();}
}
