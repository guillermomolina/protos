...
- A blank content line contains no characters other than SPACE and TAB (possibly none). Blank content lines are exempt from the prefix requirement and need not contain the complete structural indentation prefix; their SPACE and TAB characters are removed as incidental source-formatting indentation, so a source blank line contributes an empty logical line rather than whitespace caused solely by source indentation. No whitespace is removed from a non-blank content line beyond the single structural prefix.
